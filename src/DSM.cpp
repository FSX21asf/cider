...
void DSM::read_sync_without_sink(char* buffer,
                                 GlobalAddress gaddr,
                                 size_t size,
                                 CoroPull* sink,
...
void DSM::write_without_sink(const char* buffer,
                             GlobalAddress gaddr,
                             size_t size,
                             bool signal,
                             CoroPull* sink,
...
void DSM::write_sync_without_sink(const char* buffer,
                                  GlobalAddress gaddr,
                                  size_t size,
                                  CoroPull* sink,
                                  CoroQueue* waiting_queue) {
...
void DSM::read_batch_sync_without_sink(RdmaOpRegion* rs,
                                       int k,
                                       CoroPull* sink,
                                       CoroQueue* waiting_queue) {
    uint64_t wrID = sink ? sink->get() : 0ULL;
...
void DSM::write_batch_without_sink(RdmaOpRegion* rs,
                                   int k,
                                   bool signal,
                                   CoroPull* sink,
                                   CoroQueue* waiting_queue) {
...
void DSM::write_batch_sync_without_sink(RdmaOpRegion* rs,
                                        int k,
                                        CoroPull* sink,
                                        CoroQueue* waiting_queue) {
    uint64_t wrID = sink ? sink->get() : 0ULL;
...
void DSM::write_batches_sync(const std::vector<RdmaOpRegion>& rs,
                             CoroPull* sink) {
    std::map<uint64_t, std::vector<RdmaOpRegion> > each_rs;
    ...
    if (GlobalAddress{cas_ror.dest}.nodeID ==
        ...
        if (!cas_ror.is_on_chip)
            cas(GlobalAddress{cas_ror.dest}, equal, val,
                (uint64_t*)cas_ror.source, true, sink);
        else
        ...
        if (!write_ror.is_on_chip)
            write((const char*)write_ror.source, GlobalAddress{write_ror.dest},
                  write_ror.size, true, sink);
        else
            write_dm((const char*)write_ror.source,
                     GlobalAddress{write_ror.dest}, write_ror.size, true, sink);
...
bool DSM::cas_mask_sync_without_sink(GlobalAddress gaddr,
                                     uint64_t equal,
                                     uint64_t val,
                                     uint64_t* rdma_buffer,
                                     uint64_t compare_mask,
    ...
    rdmaCompareAndSwapMask(iCon->data[0][gaddr.nodeID], (uint64_t)rdma_buffer,
                           remoteInfo[gaddr.nodeID].dsmBase + gaddr.offset,
                           equal, val, iCon->cacheLKey,
                           remoteInfo[gaddr.nodeID].dsmRKey[0], compare_mask,
                           swap_mask, true, wrID);
...
void DSM::faa_field_boundary_sync(GlobalAddress gaddr,
                                  ...
                                  uint64_t mask,
                                  CoroPull* sink) {
    faa_field_boundary(gaddr, add_val, rdma_buffer, mask, true, sink);
    if (sink == nullptr) {
        ibv_wc wc;
        pollWithCQ(iCon->cq, 1, &wc);
    }
...
void DSM::faa128_boundray_sync(GlobalAddress gaddr,
                               ...
                               uint64_t* mask,
                               CoroPull* sink) {
    faa128_boundray(gaddr, add_val, rdma_buffer, mask, true, sink);
    if (sink == nullptr) {
        ibv_wc wc;
        pollWithCQ(iCon->cq, 1, &wc);
    }
...
void DSM::cas128_mask(GlobalAddress gaddr,
    ...
    if (sink == nullptr) {
        ...
        rdmaEXTcasm(iCon->data[0][gaddr.nodeID], (uint64_t)rdma_buffer,
                    ...
                    (uint64_t)equal, (uint64_t)val, iCon->cacheLKey,
                    remoteInfo[gaddr.nodeID].dsmRKey[0], (uint64_t)compare_mask,
                    (uint64_t)swap_mask, 4, true, sink->get());
        (*sink)();
    }
}
