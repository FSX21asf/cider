...
Tree::Tree(DSM* dsm, uint16_t tree_id, bool init_root)
    ...
    if (define::kIndexCacheSize > define::kHotspotBufSize + 20)
        tree_cache =
            new TreeCache(define::kIndexCacheSize - define::kHotspotBufSize,
                          dsm);  // enable hotspot idx cache
    else
    ...
    if (dsm->getMyNodeID() == 0) {
        // init root page
        auto leaf_addr =
            dsm->alloc(define::allocationLeafSize, PACKED_ADDR_ALIGN_BIT);
        ...
        max_key_idx = hopscotch_insert_locally(root_leaf->records, ghost_key,
                                               define::kValueNull);
#else
        ...
        dsm->write_sync((char*)lock_buffer, leaf_addr + lock_offset,
                        sizeof(uint64_t));
#endif
        auto encoded_leaf_buffer = (dsm->get_rbuf(nullptr)).get_leaf_buffer();
#ifdef METADATA_REPLICATION
        auto intermediate_leaf_buffer =
            (dsm->get_rbuf(nullptr)).get_leaf_buffer();
        MetadataManager::encode_node_metadata(leaf_buffer,
                                              intermediate_leaf_buffer);
        LeafVersionManager::encode_node_versions(intermediate_leaf_buffer,
                                                 encoded_leaf_buffer);
#else
        VersionManager<LeafNode, LeafEntry>::encode_node_versions(
            leaf_buffer, encoded_leaf_buffer);
#endif
...
GlobalAddress Tree::get_root_ptr_ptr() {
    return GlobalAddress{
        0, define::kRootPointerStoreOffest + sizeof(GlobalAddress) * tree_id};
}
...
inline uint64_t Tree::get_lock_info(bool is_leaf) {
    return ROUND_UP(is_leaf ? define::transLeafSize : define::transInternalSize,
                    3);
}

inline uint64_t Tree::get_mcs_lock_info(bool is_leaf) {
    return ROUND_UP(is_leaf ? define::transLeafSize : define::transInternalSize,
                    3) +
           16;
}
void Tree::lock_node(const GlobalAddress& node_addr,
    ...
    auto acquire_lock = [=](const GlobalAddress& node_addr) {
        return dsm->cas_mask_sync(node_addr + lock_offset, 0UL, ~0UL,
                                  lock_buffer, 1ULL << 63, ~0ULL, sink);
    };
...
void Tree::unlock_node(const GlobalAddress& node_addr,
                       uint64_t* lock_buffer,
                       bool is_leaf,
                       CoroPull* sink,
                       bool async) {
    ...
    auto unlock = [=](const GlobalAddress& node_addr) {
        if (async) {
            dsm->write((char*)lock_buffer, node_addr + lock_offset,
                       sizeof(uint64_t), false, sink);
        } else {
...
void Tree::insert(const Key& k, Value v, CoroPull* sink) {
    ...
    lock_res =
        local_lock_table[dsm->getMyThreadID() / define::kHandOverGroupSize]
            ->acquire_local_write_lock(k, v, &busy_waiting_queue, sink);
...
next:
    ...
    // read leaf node
    if (level == 1) {
        if (!leaf_node_insert(p, sibling_p, k, v, from_cache,
                              sink)) {  // return false if cache validation fail
            if (cache_entry)
    ...
    if (!internal_node_search(p, sibling_p, k, level, from_cache,
                              sink)) {  // return false if cache validation fail
        // cache invalidation
        assert(from_cache);
        tree_cache->invalidate(cache_entry);
...
void Tree::insert_internal(const Key& k,
                           const GlobalAddress& ptr,
                           const RootEntry& root_entry,
...
bool Tree::internal_node_search(GlobalAddress& node_addr,
...
re_read:
    dsm->read_sync(raw_internal_buffer, node_addr, define::transInternalSize,
                   sink);
    if (!VersionManager<InternalNode, InternalEntry>::decode_node_versions(
            raw_internal_buffer, internal_buffer)) {
        goto re_read;
    }
    const auto& fence_keys = node->metadata.fence_keys;
    if (from_cache && (!node->metadata.valid || k < fence_keys.lowest ||
                       k >= fence_keys.highest)) {  // cache is outdated
        return false;
    }
    ...
    std::sort(records, records + define::internalSpanSize,
              [](const InternalEntry& a, const InternalEntry& b) {
                  if (a.key == define::kkeyNull)
                      return false;
                  if (b.key == define::kkeyNull)
                      return true;
                  return a.key < b.key;
              });
#endif
...
bool Tree::leaf_node_insert(const GlobalAddress& node_addr,
    ...
    // lock node
#ifdef USE_MCS
    MCS_LOCK lock(node_addr + get_mcs_lock_info(true));
    lock.lock(sink);
#endif
    ...
    read_entry_num =
        if_lock->get_read_entry_num_from_bitmap(l_idx, true);  // [l_idx, r_idx)
...
#if (defined HOPSCOTCH_LEAF_NODE && defined VACANCY_AWARE_LOCK)
    if (hopping_read) {
        hopscotch_search(node_addr, l_idx, raw_leaf_buffer, leaf_buffer, sink,
                         read_entry_num, true);
    }
#endif
    if (!hopping_read) {
        ...
        assert((LeafVersionManager::decode_node_versions(
            raw_leaf_buffer, intermediate_leaf_buffer)));
        MetadataManager::decode_node_metadata(intermediate_leaf_buffer,
                                              leaf_buffer);
#else
        assert((VersionManager<LeafNode, LeafEntry>::decode_node_versions(
            raw_leaf_buffer, leaf_buffer)));
#endif
    }

#ifdef SIBLING_BASED_VALIDATION
    const auto& sibling_ptr =
        (leaf->metadata.sibling_ptr == GlobalAddress::Widest()
             ? GlobalAddress::Null()
             : (GlobalAddress)leaf->metadata.sibling_ptr);
    // cache validation
    if (!leaf->metadata.valid ||
        (from_cache &&
         sibling_addr != sibling_ptr)) {  // invalid || cache is outdated
    ...
    if (sibling_addr != sibling_ptr) {
        ...
        if (leaf->is_root()) {
            ...
            auto check_larger_key = [=](int l, int r) {
                for (int i = l; i < r; ++i)
                    if (leaf->records[i].key >= k)
                        return true;
                return false;
            };
            if (l_idx < r_idx)
                has_large_key |= check_larger_key(l_idx, r_idx);
            else
                has_large_key |=
                    (check_larger_key(0, r_idx) ||
                     check_larger_key(l_idx, define::leafSpanSize));
            if (!has_large_key) {
                if (((l_idx < r_idx) &&
                     (max_key_idx < l_idx || max_key_idx >= r_idx)) ||
                    (l_idx >= r_idx && max_key_idx >= r_idx &&
                     max_key_idx < l_idx)) {
                    leaf_entry_read(node_addr, max_key_idx, raw_leaf_buffer,
                                    leaf_buffer, sink, true);
                }
                auto max_key = leaf->records[max_key_idx].key;
                split_key = max_key + 1;
            } else
                split_key =
                    k + 1;  // use faked split_key to avoid turning right
#else
        ...
        if (k >= split_key) {  // should turn right
            ...
            leaf_node_insert(leaf->metadata.sibling_ptr, GlobalAddress::Null(),
                             k, v, false, sink);
            return true;
        }
    }
    ...
    if (from_cache && (!leaf->metadata.valid || k < fence_keys.lowest ||
                       k >= fence_keys.highest)) {  // cache is outdated
        unlock_node(node_addr, lock_buffer, true, sink, true);
#ifdef USE_MCS
        lock.unlock();
...
#endif
        assert(leaf->metadata.sibling_ptr != GlobalAddress::Null());
        leaf_node_insert(leaf->metadata.sibling_ptr, GlobalAddress::Null(), k,
                         v, false, sink);
        return true;
    }
    assert(k >= fence_keys.lowest);
...
#ifdef ENABLE_VAR_LEN_KV
    {
        ...
        auto block_addr =
            dsm->alloc(define::dataBlockLen, PACKED_ADDR_ALIGN_BIT);
    ...
    if (i != (int)define::leafSpanSize) {
        entry_write_and_unlock<LeafNode, LeafEntry, Value>(
            leaf, i, k, v, node_addr, lock_buffer, sink);
#ifdef USE_MCS
        lock.unlock();
#endif
        return true;
    ...
    if (!(hopscotch_insert_and_unlock(
            (LeafNode*)leaf_copy_buffer, k, v, node_addr, lock_buffer, sink,
            read_entry_num))) {  // return false(remain locked) if need split
#ifdef VACANCY_AWARE_LOCK
        // read the rest of the leaf
        if (read_entry_num < (int)define::leafSpanSize) {
            hopscotch_search(node_addr, r_idx, raw_leaf_buffer, leaf_buffer,
                             sink, define::leafSpanSize - read_entry_num, true);
        }
#endif
        // split leaf node
#if defined(USE_MCS)
        hopscotch_split_and_unlock(leaf, k, v, node_addr, lock_buffer, lock,
                                   sink);
#else
    ...
    if (!need_split) {
        ...
        node_split_and_unlock<LeafNode, LeafEntry, Value, define::leafSpanSize,
                              define::allocationLeafSize,
                              define::transLeafSize>(leaf, k, v, node_addr,
                                                     lock_buffer, 0, sink);
...
#ifdef HOPSCOTCH_LEAF_NODE
bool Tree::hopscotch_insert_and_unlock(LeafNode* leaf,
                                       const Key& k,
                                       Value v,
    ...
    auto get_entry = [=, &records](int logical_idx) -> LeafEntry& {
        return records[(logical_idx + define::leafSpanSize) %
                       define::leafSpanSize];
    };
    // caculate hash idx
...
next_hop:
    ...
    if (j < hash_idx + (int)define::neighborSize) {
        ...
        segment_write_and_unlock(leaf, hash_idx,
                                 empty_idx % define::leafSpanSize, hopped_idxes,
                                 node_addr, lock_buffer, sink);
        return true;
    }
    for (int offset = define::neighborSize - 1; offset > 0; --offset) {
        ...
        for (int z = define::neighborSize - 1; z >= 0; --z) {
            ...
            // corner case
            if (h - h_home < 0)
                h_home -= define::leafSpanSize;
            else if (h - h_home >= (int)define::neighborSize)
                h_home += define::leafSpanSize;
            assert(h_home <= h);
            if (h_home + (int)define::neighborSize > j &&
                (get_entry(h_home).hop_bitmap &
                 (1ULL << (define::neighborSize - z - 1)))) {
                h_hash_idx = h_home;
                break;
            }
        }
...
void Tree::hopscotch_split_and_unlock(LeafNode* leaf,
                                      const Key& k,
                                      Value v,
                                      const GlobalAddress& node_addr,
                                      uint64_t* lock_buffer,
    ...
    if (k < split_key)
        ...
        hopscotch_insert_locally(sibling_leaf->records, k, v);
    ...
    split_key = max_key + 1;
    if (leaf->metadata.sibling_ptr == GlobalAddress::Widest())
        leaf->metadata.sibling_ptr =
            GlobalAddress::Null();  // remove the root tag
#endif
    sibling_leaf->metadata.fence_keys =
        FenceKeys{split_key, leaf->metadata.fence_keys.highest};
    ...
    MetadataManager::encode_node_metadata(sibling_buffer,
                                          intermediate_sibling_buffer);
    LeafVersionManager::encode_node_versions(intermediate_sibling_buffer,
                                             encoded_sibling_buffer);
#else
    VersionManager<LeafNode, LeafEntry>::encode_node_versions(
        sibling_buffer, encoded_sibling_buffer);
#endif
#ifdef VACANCY_AWARE_LOCK
    ...
    auto if_sibling_lock = new (encoded_sibling_buffer + get_lock_info(true))
        VALOCK(0ULL, get_max_key_idx(sibling_leaf));  // unlock
    // update vacancy bitmap
    update_vacancy_bitmap(leaf, if_lock);
    update_vacancy_bitmap(sibling_leaf, if_sibling_lock);
    dsm->write_sync(encoded_sibling_buffer, sibling_addr,
                    define::transLeafSize + define::allocationLockSize, sink);
#else
    dsm->write_sync(encoded_sibling_buffer, sibling_addr, define::transLeafSize,
                    sink);
#endif
...
#ifdef METADATA_REPLICATION
    auto intermediate_leaf_buffer = (dsm->get_rbuf(sink)).get_leaf_buffer();
    MetadataManager::encode_node_metadata((char*)leaf,
                                          intermediate_leaf_buffer);
    LeafVersionManager::encode_node_versions(intermediate_leaf_buffer,
                                             encoded_leaf_buffer);
#else
    VersionManager<LeafNode, LeafEntry>::encode_node_versions(
        (char*)leaf, encoded_leaf_buffer);
#endif
    ...
    // no need to signal
    dsm->write(encoded_leaf_buffer, node_addr,
               define::transLeafSize + define::allocationLockSize, false, sink);
#ifdef USE_MCS
    lock.unlock();
#endif
#else
    ...
    if (is_root) {  // node is root node
        auto root_addr =
            dsm->alloc(define::allocationInternalSize, PACKED_ADDR_ALIGN_BIT);
        ...
        VersionManager<InternalNode, InternalEntry>::encode_node_versions(
            root_buffer, encoded_node_buffer);
        dsm->write_sync(encoded_node_buffer, root_addr,
                        define::transInternalSize, sink);
        // udpate root pointer
        auto cas_buffer = (dsm->get_rbuf(sink)).get_cas_buffer();
        if (dsm->cas_sync(root_ptr_ptr, RootEntry(1, node_addr),
                          RootEntry(2, root_addr), cas_buffer,
                          sink)) {  // cas root success
            printf("[INFO] new root level %d\n", 2);
            rough_height.store(2);
            return;
        } else {
            // find the internal node whose level is (level + 1), insert the
            // slibling kp into it
    ...
    if (parent_node_addr != GlobalAddress::Null() ||
        (cache_entry = tree_cache->search_ptr_from_cache(
             split_key, parent_node_addr,
             1))) {  // normal cases || in case the internal node needed to
                     // split is searched(pointed) from cache
        assert(parent_node_addr != GlobalAddress::Null());
        from_cache = (cache_entry != nullptr);
        if (internal_node_insert(parent_node_addr, split_key, sibling_addr,
                                 from_cache, 1, sink)) {
            return;
        }
    }
...
#else
    if (parent_node_addr != GlobalAddress::Null()) {
        assert(internal_node_insert(parent_node_addr, split_key, sibling_addr,
                                    false, 1, sink));
        return;
    }
#endif
...
void Tree::hopscotch_search(const GlobalAddress& node_addr,
                            int hash_idx,
    ...
    auto leaf = (LeafNode*)leaf_buffer;
    auto segment_size_r =
        std::min(entry_num, (int)define::leafSpanSize - hash_idx);
    auto segment_size_l =
        entry_num <= (int)define::leafSpanSize - hash_idx
            ? 0
            : entry_num - ((int)define::leafSpanSize - hash_idx);
    ...
    if (raw_len_l)
        read_two_segments[dsm->getMyThreadID()]++;
    ...
    re_read_2:
        ...
        auto intermediate_segment_buffer_l =
            (dsm->get_rbuf(sink)).get_segment_buffer();
        auto intermediate_segment_buffer_r =
            (dsm->get_rbuf(sink)).get_segment_buffer();
        auto [first_metadata_offset_l, new_len_l] =
            MetadataManager::get_offset_info(0, segment_size_l);
        auto [first_metadata_offset_r, new_len_r] =
            MetadataManager::get_offset_info(hash_idx, segment_size_r);
        if (for_write) {  // for locked node, consistency check is not needed
            assert((LeafVersionManager::decode_segment_versions(
                raw_segment_buffer_l, intermediate_segment_buffer_l,
                first_offset_l, segment_size_l, first_metadata_offset_l,
                new_len_l, segment_node_versions_l)));
            assert((LeafVersionManager::decode_segment_versions(
                raw_segment_buffer_r, intermediate_segment_buffer_r,
                first_offset_r, segment_size_r, first_metadata_offset_r,
                new_len_r, segment_node_versions_r)));
            assert(segment_node_versions_r == segment_node_versions_l);
        } else if (!LeafVersionManager::decode_segment_versions(
                       raw_segment_buffer_l, intermediate_segment_buffer_l,
                       first_offset_l, segment_size_l, first_metadata_offset_l,
                       new_len_l, segment_node_versions_l) ||
        ...
        bool has_metadata_l = MetadataManager::decode_segment_metadata(
            intermediate_segment_buffer_l, (char*)&(leaf->records[0]),
            first_metadata_offset_l, segment_size_l, metadata_l);
        bool has_metadata_r = MetadataManager::decode_segment_metadata(
            intermediate_segment_buffer_r, (char*)&(leaf->records[hash_idx]),
            first_metadata_offset_r, segment_size_r, metadata_r);
    ...
    re_read_1:
        dsm->read_sync(raw_segment_buffer_r, node_addr + raw_offset_r,
                       raw_len_r, sink);
        ...
        auto has_metadata = MetadataManager::decode_segment_metadata(
            intermediate_segment_buffer_r, (char*)&(leaf->records[hash_idx]),
            first_metadata_offset_r, segment_size_r, leaf->metadata);
...
#else
    auto [raw_offset_r, raw_len_r, first_offset_r] =
        VersionManager<LeafNode, LeafEntry>::get_offset_info(hash_idx,
                                                             segment_size_r);
    auto [raw_offset_l, raw_len_l, first_offset_l] =
        VersionManager<LeafNode, LeafEntry>::get_offset_info(0, segment_size_l);
    assert(segment_size_l > 0 || !raw_len_l);
    if (raw_len_l)
        read_two_segments[dsm->getMyThreadID()]++;
...
re_read:
    ...
    if (for_write) {  // for locked node, consistency check is not needed
        assert((VersionManager<LeafNode, LeafEntry>::decode_header_versions(
            raw_leaf_buffer, leaf_buffer, metadata_node_version)));
        if (segment_size_l > 0)
            assert(
                (VersionManager<LeafNode, LeafEntry>::decode_segment_versions(
                    raw_segment_buffer_l, (char*)&(leaf->records[0]),
                    first_offset_l, segment_size_l, segment_node_versions_l)));
        assert((VersionManager<LeafNode, LeafEntry>::decode_segment_versions(
            raw_segment_buffer_r, (char*)&(leaf->records[hash_idx]),
            first_offset_r, segment_size_r, segment_node_versions_r)));
        assert(metadata_node_version == segment_node_versions_r);
        if (segment_size_l > 0)
            assert(metadata_node_version == segment_node_versions_l);
        return;
    }
    // consistency check; note that: (segment_size_l > 0) => there are two
    // segments
    if (!VersionManager<LeafNode, LeafEntry>::decode_header_versions(
            raw_leaf_buffer, leaf_buffer, metadata_node_version) ||
        (segment_size_l > 0 &&
         !VersionManager<LeafNode, LeafEntry>::decode_segment_versions(
             raw_segment_buffer_l, (char*)&(leaf->records[0]), first_offset_l,
             segment_size_l, segment_node_versions_l)) ||
        !VersionManager<LeafNode, LeafEntry>::decode_segment_versions(
            raw_segment_buffer_r, (char*)&(leaf->records[hash_idx]),
            first_offset_r, segment_size_r, segment_node_versions_r) ||
...
void Tree::segment_write_and_unlock(LeafNode* leaf,
                                    int l_idx,
                                    int r_idx,
    ...
    // update max_key_idx
    auto max_key_idx = if_lock->get_max_key_idx();
    if (std::find(hopped_idxes.begin(), hopped_idxes.end(), max_key_idx) !=
        hopped_idxes.end()) {
        auto new_max_key = define::kkeyNull;
        for (int idx : hopped_idxes) {
            const auto& e = records[idx];
            if (e.key > new_max_key)
                new_max_key = e.key, max_key_idx = idx;
        }
    ...
    if (l_idx <= r_idx) {  // update with one WRITE + unlock
        ...
        // segment [l_idx, r_idx]
        auto [raw_offset, raw_len, first_offset] =
            VersionManager<LeafNode, LeafEntry>::get_offset_info(
                l_idx, r_idx - l_idx + 1);
        VersionManager<LeafNode, LeafEntry>::encode_segment_versions(
            (char*)&records[l_idx], encoded_segment_buffer, first_offset,
        ...
        auto encoded_segment_buffer_1 =
            (dsm->get_rbuf(sink)).get_segment_buffer();
        auto encoded_segment_buffer_2 =
            (dsm->get_rbuf(sink)).get_segment_buffer();
#ifdef METADATA_REPLICATION
        // segment [0, r_idx]
        ...
        LeafVersionManager::encode_segment_versions(
            intermediate_segment_buffer_1, encoded_segment_buffer_1,
            first_offset_1, hopped_idxes, 0, r_idx, first_metadata_offset_1,
            new_len_1);
        // segment [l_idx, SPAN_SIZE-1]
        auto intermediate_segment_buffer_2 =
            (dsm->get_rbuf(sink)).get_segment_buffer();
        auto [first_metadata_offset_2, new_len_2] =
            MetadataManager::get_offset_info(l_idx,
        ...
        auto [raw_offset_2, raw_len_2, first_offset_2] =
            LeafVersionManager::get_offset_info(l_idx,
                                                define::leafSpanSize - l_idx);
        LeafVersionManager::encode_segment_versions(
            intermediate_segment_buffer_2, encoded_segment_buffer_2,
            first_offset_2, hopped_idxes, l_idx, define::leafSpanSize - 1,
            first_metadata_offset_2, new_len_2);
#else
        // segment [0, r_idx]
        auto [raw_offset_1, raw_len_1, first_offset_1] =
            VersionManager<LeafNode, LeafEntry>::get_offset_info(0, r_idx + 1);
        VersionManager<LeafNode, LeafEntry>::encode_segment_versions(
            (char*)&records[0], encoded_segment_buffer_1, first_offset_1,
            hopped_idxes, 0, r_idx);
        // segment [l_idx, SPAN_SIZE-1]
        auto [raw_offset_2, raw_len_2, first_offset_2] =
            VersionManager<LeafNode, LeafEntry>::get_offset_info(
                l_idx, define::leafSpanSize - l_idx);
        VersionManager<LeafNode, LeafEntry>::encode_segment_versions(
...
bool Tree::internal_node_insert(const GlobalAddress& node_addr,
                                const Key& k,
                                const GlobalAddress& v,
                                bool from_cache,
                                uint8_t level,
                                CoroPull* sink) {
    // lock node
#ifdef USE_MCS
    MCS_LOCK lock(node_addr + get_mcs_lock_info(false));
    lock.lock(sink);
#endif
    auto lock_buffer = (dsm->get_rbuf(sink)).get_lock_buffer();
...
re_read:
    ...
    if (k >= fence_keys.highest) {  // should turn right
        ...
        internal_node_insert(node->metadata.sibling_ptr, k, v, false, level,
                             sink);
        return true;
    }
    assert(k >= fence_keys.lowest);
    ...
    for (i = 0; i < (int)define::internalSpanSize; ++i)
    ...
    if (!need_split) {
#ifdef UNORDERED_INTERNAL_NODE
        entry_write_and_unlock<InternalNode, InternalEntry, GlobalAddress>(
            node, i, k, v, node_addr, lock_buffer, sink, true);
#else
        // search for the insert position
        int insert_idx;
        for (insert_idx = 0; insert_idx < i; ++insert_idx)
            if (records[insert_idx].key > k)
                break;
        // shift and insert
        ...
        node_write_and_unlock<InternalNode, InternalEntry,
                              define::transInternalSize>(
            node, node_addr, lock_buffer, sink, true);
#ifdef USE_MCS
        lock.unlock();
#endif
#endif
        ...
        // split internal node
        node_split_and_unlock<InternalNode, InternalEntry, GlobalAddress,
                              define::internalSpanSize,
                              define::allocationInternalSize,
                              define::transInternalSize>(
            node, k, v, node_addr, lock_buffer, level, sink);
#ifdef USE_MCS
        lock.unlock();
#endif
    }
...
template <class NODE,
          ...
          int ALLOC_SIZE,
          int TRANS_SIZE>
void Tree::node_split_and_unlock(NODE* node,
                                 const Key& k,
                                 VAL v,
                                 const GlobalAddress& node_addr,
                                 uint64_t* lock_buffer,
    ...
    if (NODE::IS_LEAF) {
        ...
        if (k < split_key)
            ...
            sibling_node->records[SPAN_SIZE - m] = ENTRY(k, v);
#ifdef SIBLING_BASED_VALIDATION
        auto max_key =
            ((k < split_key && records[m - 1].key < k) ? k
                                                       : records[m - 1].key);
        split_key = max_key + 1;
#endif
    } else {
        ...
        ((InternalNode*)sibling_node)->metadata.leftmost_ptr =
            ((InternalEntry*)records)[m].ptr;
        std::fill(records + m, records + SPAN_SIZE, ENTRY::Null());
#ifdef UNORDERED_INTERNAL_NODE
        if (k < split_key)
            records[m] = ENTRY(k, v);
        else
            sibling_node->records[SPAN_SIZE - m - 1] = ENTRY(k, v);
#else
        // search for the insert position
        auto shift_and_insert = [&k, &v](ENTRY* records, int num) {
            ...
            for (insert_idx = 0; insert_idx < num; ++insert_idx)
                if (records[insert_idx].key > k)
                    break;
            // shift and insert
            for (int j = num - 1; j >= insert_idx; --j)
                records[j + 1] = records[j];
            records[insert_idx].update(k, v);
        };
        if (k < split_key)
            shift_and_insert(records, m);
        else
            shift_and_insert(sibling_node->records, SPAN_SIZE - m - 1);
#endif
    }
    // change metadata
    sibling_node->metadata.level = level;
    sibling_node->metadata.fence_keys =
        FenceKeys{split_key, node->metadata.fence_keys.highest};
    node->metadata.fence_keys.highest = split_key;
#ifdef SIBLING_BASED_VALIDATION
    if (NODE::IS_LEAF && node->metadata.sibling_ptr == GlobalAddress::Widest())
        node->metadata.sibling_ptr =
            GlobalAddress::Null();  // remove the root tag
#endif
    ...
    if (!NODE::IS_LEAF) {
        ...
        ((InternalNode*)sibling_node)->metadata.sibling_leftmost_ptr =
            ((InternalNode*)node)->metadata.sibling_leftmost_ptr;
        ((InternalNode*)node)->metadata.sibling_leftmost_ptr =
            ((InternalNode*)sibling_node)->metadata.leftmost_ptr;
    }
    // write sibling
    auto encoded_sibling_buffer = (dsm->get_rbuf(sink)).get_node_buffer<NODE>();
    VersionManager<NODE, ENTRY>::encode_node_versions(sibling_buffer,
                                                      encoded_sibling_buffer);
    ...
    VersionManager<NODE, ENTRY>::encode_node_versions((char*)node,
                                                      encoded_node_buffer);
    auto lock_offset = get_lock_info(NODE::IS_LEAF);
    assert(!(*lock_buffer & (1ULL << 63)));
#ifdef SPLIT_WRITE_UNLATCH
    memcpy(encoded_node_buffer + lock_offset, lock_buffer,
           sizeof(uint64_t));  // unlock
    // no need to signal
    dsm->write(encoded_node_buffer, node_addr,
               TRANS_SIZE + define::allocationLockSize, false, sink);
#else
    std::vector<RdmaOpRegion> rs(2);
    ...
    if (is_root) {  // node is root node
        ...
        // udpate root pointer
        auto cas_buffer = (dsm->get_rbuf(sink)).get_cas_buffer();
        if (dsm->cas_sync(root_ptr_ptr, RootEntry(level + 1, node_addr),
                          RootEntry(level + 2, root_addr), cas_buffer,
                          sink)) {  // cas root success
            printf("[INFO] new root level %d\n", (int)(level + 2));
            ...
            printf("[INFO] cas root fail, insert from new root... level=%d\n",
                   (int)root_entry.level);
            rough_height.store(root_entry.level);
            insert_internal(split_key, sibling_addr, root_entry, level + 1,
                            sink);
            return;
        }
    }
    ...
    if (parent_node_addr != GlobalAddress::Null() ||
        (cache_entry = tree_cache->search_ptr_from_cache(
             split_key, parent_node_addr,
             level + 1))) {  // normal cases || in case the internal node needed
                             // to split is searched(pointed) from cache
        assert(parent_node_addr != GlobalAddress::Null());
        from_cache = (cache_entry != nullptr);
        if (internal_node_insert(parent_node_addr, split_key, sibling_addr,
                                 from_cache, level + 1, sink)) {
            return;
        }
    }
    if (from_cache) {  // cache invalidation
    ...
    // printf("[INFO] get parent node fail, insert from root... cnt=%d\n", ++
    // cnt);
    insert_internal(split_key, sibling_addr, get_root_ptr(sink), level + 1,
                    sink);
    return;
}

template <class NODE, class ENTRY, class VAL>
void Tree::entry_write_and_unlock(NODE* node,
                                  const int idx,
                                  const Key& k,
                                  VAL v,
                                  const GlobalAddress& node_addr,
    ...
    auto get_info_and_encode_versions = [=, &entry, &metadata](int idx) {
        if (NODE::IS_LEAF) {
            ...
            MetadataManager::encode_segment_metadata(
                (char*)&entry, intermediate_segment_buffer,
                first_metadata_offset, 1,
                LeafMetadata(metadata.h_version, 0, metadata.valid,
                             metadata.sibling_ptr, metadata.fence_keys));
            auto [raw_offset, raw_len, first_offset] =
                LeafVersionManager::get_offset_info(idx);
            LeafVersionManager::encode_segment_versions(
                intermediate_segment_buffer, encoded_entry_buffer, first_offset,
                std::vector<int>{idx}, idx, idx, first_metadata_offset,
                new_len);
            return std::make_pair(raw_offset, raw_len);
        } else {
            auto [raw_offset, raw_len, first_offset] =
                VersionManager<NODE, ENTRY>::get_offset_info(idx);
            VersionManager<NODE, ENTRY>::encode_entry_versions(
                (char*)&entry, encoded_entry_buffer, first_offset);
            return std::make_pair(raw_offset, raw_len);
        }
    };
    ...
    assert(!(*lock_buffer & (1ULL << 63)));
    if (idx ==
        (NODE::IS_LEAF ? define::leafSpanSize : define::internalSpanSize) - 1) {
        auto node_size =
            NODE::IS_LEAF ? define::transLeafSize : define::transInternalSize;
        memcpy(encoded_entry_buffer + raw_len + lock_offset - node_size,
               lock_buffer, sizeof(uint64_t));  // unlock
        if (async)
            dsm->write(encoded_entry_buffer, node_addr + raw_offset,
                       raw_len + define::allocationLockSize, false, sink);
        else
...
void Tree::node_write_and_unlock(NODE* node,
                                 const GlobalAddress& node_addr,
                                 uint64_t* lock_buffer,
                                 CoroPull* sink,
                                 bool async) {
    ...
    memcpy(encoded_node_buffer + lock_offset, lock_buffer,
           sizeof(uint64_t));  // unlock
    dsm->write_sync(encoded_node_buffer, node_addr,
                    TRANS_SIZE + define::allocationLockSize, sink);
#else
    std::vector<RdmaOpRegion> rs(2);
    rs[0].source = (uint64_t)encoded_node_buffer;
    rs[0].dest = node_addr.to_uint64();
...
next:
    ...
    // read leaf node
    if (level == 1) {
        if (!leaf_node_update(p, sibling_p, k, v, from_cache,
                              sink)) {  // return false if cache validation fail
            // cache invalidation
            assert(from_cache);
            tree_cache->invalidate(cache_entry);
...
bool Tree::leaf_node_update(const GlobalAddress& node_addr,
                            const GlobalAddress& sibling_addr,
                            const Key& k,
                            Value v,
                            bool from_cache,
    ...
    if (speculative_read(
            node_addr,
            std::make_pair(hash_idx, (hash_idx + define::neighborSize) %
                                         define::leafSpanSize),
            raw_leaf_buffer, leaf_buffer, k, old_v, i, sink, true)) {
        UNUSED(old_v);
        goto update_entry;
    }
#endif
    hopscotch_search(node_addr, hash_idx, raw_leaf_buffer, leaf_buffer, sink,
                     define::neighborSize, true);
#else
#ifdef SPECULATIVE_READ
    Value old_v;
    if (speculative_read(node_addr, std::make_pair(0, define::leafSpanSize),
                         raw_leaf_buffer, leaf_buffer, k, old_v, i, sink,
                         true)) {
        UNUSED(old_v);
        goto update_entry;
    }
...
#ifdef SIBLING_BASED_VALIDATION
    UNUSED(fence_keys);
    sibling_ptr = (leaf->metadata.sibling_ptr == GlobalAddress::Widest()
                       ? GlobalAddress::Null()
                       : (GlobalAddress)leaf->metadata.sibling_ptr);
    // cache validation
    if (from_cache && (!leaf->metadata.valid ||
                       sibling_addr != sibling_ptr)) {  // cache is outdated
        unlock_node(node_addr, lock_buffer, true, sink, true);
#ifdef USE_MCS
        lock.unlock();
#endif
    ...
    if (k >= fence_keys.highest) {  // should turn right
        ...
        leaf_node_update(leaf->metadata.sibling_ptr, GlobalAddress::Null(), k,
                         v, false, sink);
        return true;
    }
    assert(k >= fence_keys.lowest);
...
#ifdef SIBLING_BASED_VALIDATION
    // turn right check
    if (j == (int)define::neighborSize &&
        sibling_addr != sibling_ptr) {  // search sibling node
        unlock_node(node_addr, lock_buffer, true, sink, true);
#ifdef USE_MCS
        lock.unlock();
#endif
...
#endif
    entry_write_and_unlock<LeafNode, LeafEntry, Value>(leaf, i, k, v, node_addr,
                                                       lock_buffer, sink);
#ifdef USE_MCS
    lock.unlock();
#endif
...
bool Tree::search(const Key& k, Value& v, CoroPull* sink) {
    ...
    lock_res =
        local_lock_table[dsm->getMyThreadID() / define::kHandOverGroupSize]
            ->acquire_local_read_lock(k, &busy_waiting_queue, sink);
...
next:
    ...
    // read leaf node
    if (level == 1) {
        if (!leaf_node_search(p, sibling_p, k, v, from_cache,
                              sink)) {  // return false if cache validation fail
            // cache invalidation
            assert(from_cache);
            tree_cache->invalidate(cache_entry);
        ...
        if (search_res) {
            ...
            auto block_buffer = (dsm->get_rbuf(sink)).get_block_buffer();
            dsm->read_sync(block_buffer, (GlobalAddress)block_addr, block_len,
                           sink);
...
#ifdef TREE_ENABLE_READ_DELEGATION
    local_lock_table[dsm->getMyThreadID() / define::kHandOverGroupSize]
        ->release_local_read_lock(k, lock_res, search_res,
                                  v);  // handover the ret leaf addr
#endif
    return search_res;
}

void Tree::leaf_entry_read(const GlobalAddress& leaf_addr,
                           const int idx,
                           char* raw_leaf_buffer,
                           char* leaf_buffer,
                           CoroPull* sink,
...
#else
    auto [raw_offset, raw_len, first_offset] =
        VersionManager<LeafNode, LeafEntry>::get_offset_info(idx);
    auto raw_entry_buffer = raw_leaf_buffer + raw_offset;
re_read:
    // read metadata and the hop segment
    std::vector<RdmaOpRegion> rs(2);
    ...
    if (for_write) {  // for locked node, consistency check is not needed
        assert((VersionManager<LeafNode, LeafEntry>::decode_header_versions(
            raw_leaf_buffer, leaf_buffer, metadata_node_version)));
        assert((VersionManager<LeafNode, LeafEntry>::decode_segment_versions(
            raw_entry_buffer, (char*)&(leaf->records[idx]), first_offset, 1,
            entry_node_version)));
...
#ifdef SPECULATIVE_READ
bool Tree::speculative_read(const GlobalAddress& leaf_addr,
                            ...
                            bool for_write) {
    auto leaf = (LeafNode*)leaf_buffer;
    if (idx_cache->search_idx_from_cache(leaf_addr, range.first, range.second,
                                         k, speculative_idx)) {
        // read entry
        try_speculative_read[dsm->getMyThreadID()]++;
        leaf_entry_read(leaf_addr, speculative_idx, raw_leaf_buffer,
                        leaf_buffer, sink, for_write);
...
bool Tree::leaf_node_search(const GlobalAddress& node_addr,
                            const GlobalAddress& sibling_addr,
                            const Key& k,
                            Value& v,
                            bool from_cache,
...
re_read:
    ...
    int speculative_idx;
    if (speculative_read(node_addr, std::make_pair(0, define::leafSpanSize),
                         raw_leaf_buffer, leaf_buffer, k, v, speculative_idx,
                         sink)) {
        UNUSED(speculative_idx);
        return true;
    }
...
re_read:
    dsm->read_sync(raw_leaf_buffer, node_addr, define::transLeafSize, sink);
    if (!VersionManager<LeafNode, LeafEntry>::decode_node_versions(
            raw_leaf_buffer, leaf_buffer)) {
        read_leaf_retry[dsm->getMyThreadID()]++;
        goto re_read;
    }
    ...
    if (k >= fence_keys.highest) {  // should turn right
        assert(leaf->metadata.sibling_ptr != GlobalAddress::Null());
        leaf_read_sibling[dsm->getMyThreadID()]++;
        leaf_node_search(leaf->metadata.sibling_ptr, GlobalAddress::Null(), k,
                         v, false, sink);
        return true;
    }
    assert(k >= fence_keys.lowest);
    ...
    for (int i = 0; i < (int)define::neighborSize; ++i) {
        ...
        if (e.key != define::kkeyNull &&
            ...
            if (e.key == k) {  // optimization: if the target key is found,
                ...
                v = e.value;
#ifdef SPECULATIVE_READ
                idx_cache->add_to_cache(
                    node_addr, (hash_idx + i) % define::leafSpanSize, k);
#endif
                return true;
            }
        }
...
/*
  ...
  SHOULD be called with other tree optimizations (e.g., HOPSCOTCH_LEAF_NODE,
  METADATA_REPLICATION) turned on
*/
bool Tree::range_query(const Key& from,
                       const Key& to,
                       std::map<Key, Value>& ret) {  // [from, to)
    assert(dsm->is_register());
    ...
    using InfoMap =
        std::map<uint64_t,
                 std::vector<std::tuple<int, int, GlobalAddress, uint64_t,
                                        uint64_t, uint64_t> > >;
    thread_local InfoMap
        leaf_info;  // [leaf_id, (hash_idx, seg_size, leaf_addr, raw_offset,
                    // raw_len, first_offset) * N]
#ifdef SPECULATIVE_READ
    ...
    // FIXME: for simplicity, we assume all innernal nodes are cached in compute
    // node like Sherman
    ...
    for (const auto& node : cache_search_result) {
        ...
        bool no_fetch =
            from >= records[0].key || to <= metadata.fence_keys.lowest;
        if (!no_fetch) {
            leaf_addrs.insert(metadata.leftmost_ptr);
            leaf_fences[metadata.leftmost_ptr] =
                FenceKeys(metadata.fence_keys.lowest, records[0].key);
        }
        int i;
        for (i = 1; i <= (int)define::internalSpanSize; ++i) {
            if (i == (int)define::internalSpanSize ||
                records[i].key == define::kkeyNull) {
                no_fetch = from >= metadata.fence_keys.highest ||
                           to <= records[i - 1].key;
                if (!no_fetch) {
                    leaf_addrs.insert(records[i - 1].ptr);
                    leaf_fences[records[i - 1].ptr] = FenceKeys(
                        records[i - 1].key, metadata.fence_keys.highest);
                }
                break;
            }
            ...
            if (!no_fetch) {
                leaf_addrs.insert(records[i - 1].ptr);
                leaf_fences[records[i - 1].ptr] =
                    FenceKeys(records[i - 1].key, records[i].key);
            }
        }
    }

    auto merge_internals =
        [](std::vector<std::pair<int, int> >& intervals,
           std::vector<std::pair<int, int> >& res) {  // intervals: [l, r)
            std::sort(
            ...
            while (i < n) {
                ...
                while (j < n && intervals[j].first <= end)
            ...
            while (i < n) {
                ...
                while (j < n &&
                       std::max(end, temp[j].second) <=
                           (temp[i].first + (int)define::maxLeafEntryPerIO))
                    end = std::max(end, temp[j++].second);
    ...
    for (const auto& leaf_addr : leaf_addrs) {
        ...
        if (l_k == leaf_fences[leaf_addr].lowest &&
            r_k == leaf_fences[leaf_addr].highest) {
            segments.emplace_back(std::make_pair(0, (int)define::leafSpanSize));
        } else
            for (auto k = l_k; k < r_k; k = k + 1) {
                int hash_idx = get_hashed_leaf_entry_index(k);
#ifdef SPECULATIVE_READ
                try_read_leaf[dsm->getMyThreadID()]++;
                int speculative_idx;
                if (idx_cache->search_idx_from_cache(
                        leaf_addr, hash_idx,
                    ...
                    segments.emplace_back(
                        std::make_pair(speculative_idx, speculative_idx + 1));
                    continue;
                }
#endif
                if (hash_idx + (int)define::neighborSize <=
                    (int)define::leafSpanSize)
                    segments.emplace_back(std::make_pair(
                        hash_idx, hash_idx + (int)define::neighborSize));
                else {
                    segments.emplace_back(
                        std::make_pair(hash_idx, (int)define::leafSpanSize));
                    segments.emplace_back(std::make_pair(
                        0, (int)define::neighborSize -
                               ((int)define::leafSpanSize - hash_idx)));
                }
            }
        // merge the intervals
        ...
        for (const auto& [l_idx, r_idx] : merged_segments) {
            // get info
            auto [raw_offset, raw_len, first_offset] =
                LeafVersionManager::get_offset_info(l_idx, r_idx - l_idx);
            RdmaOpRegion r;
            r.source = (uint64_t)range_buffer +
                       leaf_cnt * define::allocationLeafSize + raw_offset;
            r.dest = (leaf_addr + raw_offset).to_uint64();
            r.size = raw_len;
            r.is_on_chip = false;
            rs.push_back(r);
            leaf_info[leaf_cnt].emplace_back(
                std::make_tuple(l_idx, r_idx - l_idx, leaf_addr, raw_offset,
                                raw_len, first_offset));
        }
        ++leaf_cnt;
        assert(
            (dsm->get_rbuf(nullptr))
                .is_safe(range_buffer + leaf_cnt * define::allocationLeafSize));
    }
    int next_leaf_cnt;
    InfoMap next_info;
    // batch read
re_read:
    ...
    for (int i = 0; i < leaf_cnt; ++i) {
        auto leaf_buffer = (dsm->get_rbuf(nullptr)).get_leaf_buffer();
        auto leaf = (LeafNode*)leaf_buffer;
        for (const auto& [start_idx, segment_size, leaf_addr, raw_offset,
                          raw_len, first_offset] : leaf_info[i]) {
            auto raw_buffer =
                range_buffer + i * define::allocationLeafSize + raw_offset;
            uint8_t segment_node_versions = 0;
            auto intermediate_buffer =
                (dsm->get_rbuf(nullptr)).get_segment_buffer();
            auto [first_metadata_offset, new_len] =
                MetadataManager::get_offset_info(start_idx, segment_size);
            // check versions consistency
            if (!(LeafVersionManager::decode_segment_versions(
                    raw_buffer, intermediate_buffer, first_offset, segment_size,
                    first_metadata_offset, new_len, segment_node_versions))) {
                RdmaOpRegion r;
                r.source = (uint64_t)range_buffer +
                           next_leaf_cnt * define::allocationLeafSize +
                           raw_offset;
                r.dest = (leaf_addr + raw_offset).to_uint64();
                r.size = raw_len;
                r.is_on_chip = false;
                rs.push_back(r);
                next_info[next_leaf_cnt].emplace_back(
                    std::make_tuple(start_idx, segment_size, leaf_addr,
                                    raw_offset, raw_len, first_offset));
                continue;
            }
            MetadataManager::decode_segment_metadata(
                intermediate_buffer, (char*)&(leaf->records[start_idx]),
                first_metadata_offset, segment_size, leaf->metadata);
            // check hopping consistency
            std::vector<int> hash_idxes;
            for (int j = 0; j < segment_size; ++j) {
                const auto& e = leaf->records[start_idx + j];
                if (e.key == define::kkeyNull)
                    hash_idxes.emplace_back(-1);
                else
                    hash_idxes.emplace_back(get_hashed_leaf_entry_index(e.key));
            }
            bool is_ok = true;
            for (int j = 0; j < segment_size && is_ok; ++j) {
                const auto& hop_bitmap =
                    leaf->records[start_idx + j].hop_bitmap;
                for (int k = 0;
                     k < std::min((int)define::neighborSize, segment_size - j);
                     ++k) {
                    if ((hop_bitmap &
                         (1ULL << (define::neighborSize - k - 1))) &&
                        hash_idxes[j + k] != start_idx + j) {
                        is_ok = false;
                        break;
                    }
                }
        ...
        if (next_info.find(next_leaf_cnt) != next_info.end())
            next_leaf_cnt++;
    }
#else
    ...
    for (const auto& leaf_addr : leaf_addrs) {
        RdmaOpRegion r;
        r.source =
            (uint64_t)range_buffer + leaf_cnt * define::allocationLeafSize;
...
re_read:
    ...
    for (int i = 0; i < leaf_cnt; ++i) {
        ...
        auto intermediate_leaf_buffer =
            (dsm->get_rbuf(nullptr)).get_leaf_buffer();
        if (!LeafVersionManager::decode_node_versions(
                raw_leaf_buffer, intermediate_leaf_buffer)) {
            RdmaOpRegion r;
            r.source = (uint64_t)range_buffer +
                       next_leaf_cnt * define::allocationLeafSize;
    ...
    for (const auto& k : speculative_keys) {
        if (ret.find(k) != ret.end())
            ...
            uint16_t level;
            auto cache_entry =
                tree_cache->search_from_cache(k, p, sibling_p, level);
    ...
    for (const auto& [leaf_addr, keys] : retry_leaf_keys) {
        ...
        for (const auto& [l_idx, r_idx] : merged_segments) {
            ...
            RdmaOpRegion r;
            r.source = (uint64_t)range_buffer +
                       next_leaf_cnt * define::allocationLeafSize + raw_offset;
            r.dest = (leaf_addr + raw_offset).to_uint64();
            r.size = raw_len;
            r.is_on_chip = false;
            rs.push_back(r);
    ...
    assert((dsm->get_rbuf(nullptr))
               .is_safe(range_buffer + kv_cnt * define::dataBlockLen));
...
#endif
    // FIXME: for simplicity, load uncached innernal nodes according to ret,
    // leveraging the ordered YCSB E
    for (auto k = from; k < to; k = k + 1) {
        if (ret.find(k) != ret.end()) {
...
void Tree::run_coroutine(GenFunc gen_func,
                         WorkFunc work_func,
                         int coro_cnt,
                         Request* req,
                         int req_num) {
