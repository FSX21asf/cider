...
extern uint64_t array_write_handover_num[MAX_APP_THREAD];
extern uint64_t array_pessimistic_num[MAX_APP_THREAD];
...
extern uint64_t array_try_write_op[MAX_APP_THREAD];
...
extern volatile bool array_need_stop;
extern volatile bool array_need_clear[MAX_APP_THREAD];
extern uint64_t array_latency[MAX_APP_THREAD][MAX_CORO_NUM][LATENCY_WINDOWS];
...
void work_func(MiniArray* ar, const Request& r, CoroPull* sink) {
    if (r.req_type == SEARCH) {
        Value v;
        ar->search(key2int(r.k), v, sink);
    } else {
        ar->insert(key2int(r.k), r.v, ECPolicy::InsUdp, sink);
    }
}
...
void thread_load(int id) {
    uint64_t all_loader_thread =
        std::min(kThreadCount, 64) * dsm->getClusterSize();
    uint64_t loader_id = std::min(kThreadCount, 64) * dsm->getMyNodeID() + id;
    ...
    for (uint64_t i = 0; i < end_warm_key; ++i) {
        if (i % all_loader_thread == loader_id) {
            arr->insert(i, i * 2, ECPolicy::InsUdp);
        }
    }
...
void thread_run(int id) {
    ...
    while (!array_need_stop) {
        auto r = gen->next();

        timer.begin();
        work_func(arr, r, nullptr);
        auto us_100 = timer.end() / 1000;

        if (us_100 >= LATENCY_WINDOWS) {
            us_100 = LATENCY_WINDOWS - 1;
        }
        array_latency[thread_id][0][us_100]++;
    }
#endif
}
...
void parse_args(int argc, char* argv[]) {
    if (argc != 6) {
        printf(
            "Usage: ./array_zipfian_test kNodeCount kReadRatio kThreadCount "
            "zipfan "
            "kCoroCnt\n");
        exit(-1);
    }
    ...
    for (int i = 0; i < LATENCY_WINDOWS; ++i) {
        ...
        for (int k = 0; k < MAX_APP_THREAD; ++k)
            for (int j = 0; j < MAX_CORO_NUM; ++j) {
                latency_th_all[i] += array_latency[k][j][i];
            }
    }
    // store in file
...
int main(int argc, char* argv[]) {
    ...
    while (!array_need_stop) {
        ...
        uint64_t try_write_op_cnt = 0, write_handover_cnt = 0,
                 possible_write_handover_cnt = 0, tot_wc_pack_cnt = 0,
                 tot_pessimistic_cnt = 0;
        for (int i = 0; i < MAX_APP_THREAD; ++i) {
            tot_pessimistic_cnt += array_pessimistic_num[i];
            write_handover_cnt += array_write_handover_num[i];
            possible_write_handover_cnt += mcs_possible_write_handover_num[i];
            possible_write_handover_cnt += array_write_handover_num[i];
            try_write_op_cnt += array_try_write_op[i];
            tot_wc_pack_cnt += exactly_write_handover_pack_count[i];
        }
        // printf("debug:%lx %lx\n", write_handover_cnt, try_write_op_cnt);
        ...
        std::fill(array_need_clear, array_need_clear + MAX_APP_THREAD, true);

        save_latency(++count);
        if (count >= TEST_EPOCH) {
            array_need_stop = true;
        }
        ...
        if (dsm->getMyNodeID() == 0) {
            ...
            printf("possible combining rate: %.4lf\n",
                   possible_write_handover_cnt * 1.0 / try_write_op_cnt);
            if (tot_wc_pack_cnt) {
                printf("avg. wcpack size: %.4lf\n",
                       write_handover_cnt * 1.0 / tot_wc_pack_cnt + 1);
            } else {
                printf("avg. wcpack size: %.4lf\n", 1.0);
            }
            printf("pessimistic rate: %.4lf\n",
                   tot_pessimistic_cnt * 1.0 / try_write_op_cnt);
            printf("\n");
        }
    }
    printf("[END]\n");
