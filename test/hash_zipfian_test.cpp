...
extern uint64_t hash_write_handover_num[MAX_APP_THREAD];
...
extern uint64_t hash_try_write_op[MAX_APP_THREAD];
...
extern volatile bool hash_need_stop;
extern volatile bool hash_need_clear[MAX_APP_THREAD];
extern uint64_t hash_latency[MAX_APP_THREAD][MAX_CORO_NUM][LATENCY_WINDOWS];
...
void work_func(RaceHash* tb, const Request& r, CoroPull* sink) {
    if (r.req_type == SEARCH) {
        Value v;
        tb->search(r.k, v, sink);
    } else {
        tb->insert(r.k, r.v, ECPolicy::InsUdp, sink);
    }
}
...
void thread_load(int id) {
    ...
    for (uint64_t i = 0; i < end_warm_key; ++i) {
        if (i % all_loader_thread == loader_id) {
            hst->insert(to_key(i), i * 2, ECPolicy::InsUdp, 0);
        }
    }
...
void parse_args(int argc, char* argv[]) {
    if (argc != 6) {
        printf(
            "Usage: ./hash_zipfian_test kNodeCount kReadRatio kThreadCount "
            "zipfan "
            "kCoroCnt\n");
        exit(-1);
    }
    ...
    for (int i = 0; i < LATENCY_WINDOWS; ++i) {
        ...
        for (int k = 0; k < MAX_APP_THREAD; ++k)
            for (int j = 0; j < MAX_CORO_NUM; ++j) {
                latency_th_all[i] += hash_latency[k][j][i];
            }
    }
    // store in file
...
int main(int argc, char* argv[]) {
    ...
    while (!hash_need_stop) {
        ...
        uint64_t try_write_op_cnt = 0, possible_write_handover_cnt = 0,
                 write_handover_cnt = 0, tot_wc_pack_cnt = 0;
        for (int i = 0; i < MAX_APP_THREAD; ++i) {
            write_handover_cnt += hash_write_handover_num[i];
            possible_write_handover_cnt += mcs_possible_write_handover_num[i];
            possible_write_handover_cnt += hash_write_handover_num[i];
            try_write_op_cnt += hash_try_write_op[i];
            write_handover_cnt += hash_write_handover_num[i];
        ...
        std::fill(hash_need_clear, hash_need_clear + MAX_APP_THREAD, true);
