...
class CacheHeader {
   public:
    ...
    CacheHeader(const std::vector<uint8_t>& byte_array,
                int depth,
                int partial_len)
        : depth(depth) {
...
class CacheNode {
   public:
    ...
    // insert leaf node
    CacheNode(const std::vector<uint8_t>& byte_array,
              int start,
              CacheEntry* new_entry) {
        header =
            new CacheHeader(byte_array, start, byte_array.size() - start - 1);
    ...
    CacheNode(const std::vector<uint8_t>& byte_array,
              ...
              uint8_t partial_2,
              CacheEntry* new_entry,
              CacheNode*& nested_node) {
        header = new CacheHeader(byte_array, start, partial_len);
        if (partial_1 ==
            partial_2) {  // split for insert new_entry at old header
            ...
            if (start + partial_len >=
                ...
                nested_node = new CacheNode(byte_array, start + partial_len + 1,
                                            new_entry);
...
struct SearchRet {
    ...
    // uint64_t counter;
    SearchRet() {}
    SearchRet(volatile CacheEntry** entry_ptr_ptr,
              CacheEntry* entry_ptr,
              int next_idx)
...
class RadixCache {
   public:
    ...
    void add_to_cache(const Key& k,
                      const InternalPage* p_node,
                      const GlobalAddress& node_addr);

    bool search_from_cache(const Key& k,
                           volatile CacheEntry**& entry_ptr_ptr,
                           CacheEntry*& entry_ptr,
                           int& entry_idx);
