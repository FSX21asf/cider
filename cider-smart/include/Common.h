...
#define MAX_APP_THREAD \
    65  // one additional thread for data statistics(main thread)  [config]
...
using CRCProcessor = boost::crc_optimal<64,
                                        0x42F0E1EBA9EA3693,
                                        0xffffffffffffffff,
                                        0xffffffffffffffff,
                                        false,
...
constexpr int64_t kPerThreadRdmaBuf =
    rdmaBufferSize * define::GB / MAX_APP_THREAD;
...
#ifdef SVSIZE
constexpr uint32_t simulatedValLen = SVSIZE;
#else
constexpr uint32_t simulatedValLen = 8;
#endif
constexpr uint32_t allocAlignLeafSize =
    ROUND_UP(keyLen + simulatedValLen + 8 + 2, ALLOC_ALLIGN_BIT);
...
// FIXME: APPLOCK_BYTE
constexpr uint64_t kAPPThreadLockBytesOffset = kChunkSize;
...
#else
constexpr uint64_t kHandOverGroupSize = 8;
#endif

#ifdef ST_LIM
constexpr uint64_t kSTlimit = ST_LIM;
#else
constexpr uint64_t kSTlimit = 1;
#endif
#ifdef ST_SENTI
...
constexpr uint64_t kReserveChunk = 1 +                       // legacy chunk 0
                                   kLockBufferChunkPerNode;  // chunk 1
// Internal Node
