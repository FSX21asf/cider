   ...
   public:
    ...
    bool is_consistent() const {
        ...
        crc_processor.process_bytes(
            (char*)&key,
            sizeof(Key) + sizeof(uint8_t) * define::simulatedValLen);
    ...
    void lock() { w_lock = 1; };
    uint64_t valueOffset() const { return (uint64_t)&value - (uint64_t)this; }
...
class Header {
   ...
   public:
    ...
    Header(NodeType node_type)
        : depth(0), node_type(node_type), partial_len(0) {
        memset(partial, 0, sizeof(uint8_t) * define::hPartialLenMax);
    }
...
class InternalEntry {
   ...
   public:
    ...
    InternalEntry(uint8_t partial,
                  NodeType node_type,
                  const GlobalAddress& addr)
        : partial(partial),
          empty(0),
          node_type(static_cast<uint8_t>(node_type)),
          is_leaf(0),
          packed_addr{addr.nodeID, addr.offset >> ALLOC_ALLIGN_BIT} {}
...
class InternalPage {
   ...
   public:
    ...
    bool is_valid(const GlobalAddress& p_ptr,
                  int depth,
...
struct ScanContext {
    ...
    State r_state;
    ScanContext() {}
    ScanContext(const InternalEntry& e,
                const GlobalAddress& e_ptr,
                int depth,
                bool from_cache,
                volatile CacheEntry** entry_ptr_ptr,
