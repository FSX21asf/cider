...
bool rdmaWrite(ibv_qp* qp,
    ...
    if (size <=
        kInlineDataMax) {  // Optimization: write inline for small node/leaf
...
bool rdmaFetchAndAddWithFieldBoundary(ibv_qp* qp,
                                      uint64_t source,
                                      uint64_t dest,
                                      uint64_t add,
                                      uint32_t lkey,
    ...
    auto& op = wr.ext_op.masked_atomics.wr_data.inline_data.op.fetch_add;
    op.add_val = add;
    op.field_boundary = field_boundary;
...
bool rdmaEXTfaaB(ibv_qp* qp,
    ...
    fillSgeWr(sg, wr, source, 1 << log_arg_sz, lkey);
    ...
    wr.ext_op.masked_atomics.log_arg_sz = log_arg_sz;
    wr.ext_op.masked_atomics.remote_addr = dest;
    wr.ext_op.masked_atomics.rkey = remoteRKey;
...
bool rdmaWriteCasMask(ibv_qp* qp,
                      const RdmaOpRegion& write_ror,
                      const RdmaOpRegion& cas_ror,
                      uint64_t compare,
                      uint64_t swap,
...
bool rdmaTwoCasMask(ibv_qp* qp,
                    const RdmaOpRegion& cas_ror_1,
                    uint64_t compare_1,
                    uint64_t swap_1,
                    uint64_t mask_1,
                    const RdmaOpRegion& cas_ror_2,
                    uint64_t compare_2,
                    uint64_t swap_2,
                    uint64_t mask_2,
                    bool isSignaled,
