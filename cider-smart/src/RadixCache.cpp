...
void RadixCache::add_to_cache(const Key& k,
                              const InternalPage* p_node,
...
next:
    ...
    for (int i = 0; i < (int)hdr->partial.size(); ++i) {
        ...
        if (hdr->depth + i == (int)byte_array.size() - 1 ||
            ...
            auto new_node =
                new CacheNode(byte_array, idx, partial_len, hdr->partial[i],
                              node, cur_partial, new_entry, nested_node);
            auto& parent_node_entry = parent_node->records[byte_array[idx - 1]];
            auto ret_node = __sync_val_compare_and_swap(
                &(parent_node_entry.next), node, new_node);
    ...
    if (idx == (int)byte_array.size() - 1) {
        ...
        if (__sync_bool_compare_and_swap(&(node_entry.cache_entry), old_entry,
            ...
            eviction_list.push(
                std::make_pair(&(node_entry.cache_entry), new_entry));
        } else {
            delete new_entry;
        }
        return;
    ...
    // 2.2 internal level
        ...
        if (node_entry.next == nullptr) {
            ...
            if (ret_node == 0UL) {  // cas success
                ...
                CacheMap::const_iterator tmp =
                    next_node->records.find(byte_array.back());
                eviction_list.push(
                    std::make_pair(&(tmp->second.cache_entry), new_entry));
                free_manager->consume_by_node(node);
...
next:
    ...
    if (r_entry != cache_map.end()) {
        ...
        // cache_entry), idx + 1));
        ret.push(
            SearchRet(&(r_entry->second.cache_entry), cache_entry, idx + 1));
...
void RadixCache::search_range_from_cache(const Key& from,
                                         const Key& to,
