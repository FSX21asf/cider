...
void NormalCache::add_to_cache(const Key& k,
...
void NormalCache::_insert(const CacheKey& byte_array, CacheEntry* new_entry) {
    ...
    if (__sync_bool_compare_and_swap(&(cache_map[byte_array]), old_entry,
        ...
        //       if (free_size.compare_exchange_weak(cur_free_size,
        //       cur_free_size - (new_map_size - old_map_size))) {
...
bool NormalCache::_search(CacheKey& byte_prefix,
                          uint8_t last_byte,
                          volatile CacheEntry**& entry_ptr_ptr,
                          CacheEntry*& entry_ptr,
                          int& entry_idx) {
...
void NormalCache::search_range_from_cache(const Key& from,
                                          const Key& to,
