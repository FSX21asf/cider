...
extern uint64_t possible_write_handover_num[MAX_APP_THREAD];
...
void Tree::insert(const Key& k,
    ...
    lock_res =
        local_lock_table[(dsm->getMyThreadID() - 1) /
                         define::kHandOverGroupSize]
            ->acquire_local_write_lock(k, v, &busy_waiting_queue, cxt, coro_id);
...
next:
    ...
    if (p.is_leaf) {
        ...
        if (!is_valid) {
            ...
            // re-read leaf entry
            auto entry_buffer = (dsm->get_rbuf(coro_id)).get_entry_buffer();
            dsm->read_sync((char*)entry_buffer, p_ptr, sizeof(InternalEntry),
                           cxt);
        ...
        if (_k == k) {
            ...
            bool res =
                out_of_place_update_leaf(k, v, depth, leaf_addr, p_ptr, p,
    ...
    for (int i = 0; i < hdr.partial_len; ++i) {
        if (get_partial(k, hdr.depth + i) != hdr.partial[i]) {
            ...
            auto cas_buffer = (dsm->get_rbuf(coro_id)).get_cas_buffer();
            int partial_len =
                hdr.depth + i - depth;  // hdr.depth may be outdated, so use
                                        // partial_len wrt. depth
            bool res = out_of_place_write_node(
                k, v, depth, leaf_addr, partial_len, hdr.partial[i], p_ptr, p,
                node_ptr, cas_buffer, cxt, coro_id);
            // cas fail, retry
            if (!res) {
                p = *(InternalEntry*)cas_buffer;
            ...
            dsm->cas_mask(GADD(p.addr(), sizeof(GlobalAddress)), (uint64_t)hdr,
                          ...
                          ~Header::node_type_mask, ~Header::node_type_mask,
                          false, cxt);
            goto insert_finish;
        }
    }
    ...
    for (int i = 0; i < max_num; ++i) {
        ...
        if (old_e == InternalEntry::Null()) {
            auto e_ptr = GADD(p.addr(), sizeof(GlobalAddress) + sizeof(Header) +
                                            i * sizeof(InternalEntry));
            auto cas_buffer = (dsm->get_rbuf(coro_id)).get_cas_buffer();
            bool res = out_of_place_write_leaf(
                k, v, depth + 1, leaf_addr, get_partial(k, depth), e_ptr, old_e,
                node_ptr, cas_buffer, cxt, coro_id);
            // cas success, return
            if (res) {
                goto insert_finish;
            ...
            // cas fail, check
                ...
                if (e.partial ==
                    get_partial(k, depth)) {  // same partial keys insert to the
                                              // same empty slot
                    p_ptr = e_ptr;
                    p = e;
                    from_cache = false;
    ...
    if (insert_behind(k, v, depth + 1, leaf_addr, get_partial(k, depth),
                      p.type(), node_ptr, cas_buffer, slot_id, cxt,
...
bool Tree::read_leaf(const GlobalAddress& leaf_addr,
                     char* leaf_buffer,
                     int leaf_size,
                     const GlobalAddress& p_ptr,
                     bool from_cache,
                     CoroContext* cxt,
                     int coro_id) {
...
thread_local std::map<uint64_t, int> Tree::retryCnt, Tree::useLockCnt;
void Tree::in_place_update_leaf(const Key& k,
                                Value& v,
    ...
    local_lock_table[(dsm->getMyThreadID() - 1) / define::kHandOverGroupSize]
        ->get_combining_value(k, v);
#endif
    constexpr uint64_t LeafAlign16 = (sizeof(Leaf) + 15) / 16 * 16;
    GlobalAddress lockaddr = GADD(leaf_addr, LeafAlign16);
    GlobalAddress leafV = GADD(leaf_addr, leaf->valueOffset());
    bool wd = false;
    GlobalAddress vaddr;
    uint64_t* tmp;
#ifdef DELAY_WRITE
    ...
    *tmp = v;
    dsm->write_sync((char*)tmp, vaddr, 8, cxt);
#endif
    DATA_POINTER pre;
    pre.val = leaf->get_value();
    bool useLock = true;
    uint64_t idx = key2int(k);
#ifdef USE_ATOMIC
    useLock = false;
#endif
#ifdef USE_ST
...
#endif
#ifdef USE_MCS_WC
    MCS_WC_LOCK lk(lockaddr);
    if (useLock) {
        if (lk.lock(0, cxt, coro_id)) {
            write_handover_num[dsm->getMyThreadID()]++;
            return;
        }
    }
#endif
#ifdef USE_MCS_WCV
    MCS_WC_VLOCK lk(lockaddr, leafV);
    if (useLock) {
        uint64_t ret = lk.lock(0, pre.version, cxt, coro_id);
        assert(ret != 3);  // no delete,if has delete and update turn into
                           // insert,maybe go ahead
        if (ret) {
            write_handover_num[dsm->getMyThreadID()]++;
            return;
        }
    ...
    DATA_POINTER ne;
    ne.nodeID = vaddr.nodeID;
    ne.offset = vaddr.offset;
    ne.version = pre.version;
    tmp = (dsm->get_rbuf(coro_id)).get_cas_buffer();
    while (!dsm->cas_sync(leafV, pre.to_uint64(), ne.val, tmp, cxt)) {
...
#ifdef USE_ST
    if (!useLock) {
        if (retryCnt[idx] >= define::kSTlimit && retryc >= define::kSTlimit) {
...
#ifdef USE_MCS_WC
    if (useLock) {
        uint64_t sp = lk.unlock(cxt, coro_id);
        if (sp) {
            useLockCnt[idx] = useLockCnt[idx] / 2;
        } else {
            useLockCnt[idx] += 2;
        }
    }
    ...
    // #ifdef TREE_ENABLE_EMBEDDING_LOCK
    //         return dsm->cas_mask_sync(GADD(unique_leaf_addr,
    //         lock_cas_offset), 0UL,
    //                                   ~0UL, cas_buffer, lock_mask, lock_mask,
    //                                   cxt);
    // #else
    //         GlobalAddress lock_addr;
    //         uint64_t mask;
    //         get_on_chip_lock_addr(unique_leaf_addr, lock_addr, mask);
    //         return dsm->cas_dm_mask_sync(lock_addr, 0UL, ~0UL, cas_buffer,
    //         mask,
    //                                      mask, cxt);
    // #endif
    //     };
    ...
    // #ifdef TREE_ENABLE_EMBEDDING_LOCK
    //         dsm->cas_mask_sync(GADD(unique_leaf_addr, lock_cas_offset), ~0UL,
    //         0UL,
    //                            cas_buffer, lock_mask, lock_mask, cxt);
    // #else
    //         GlobalAddress lock_addr;
    //         uint64_t mask;
    //         get_on_chip_lock_addr(unique_leaf_addr, lock_addr, mask);
    //         dsm->cas_dm_mask_sync(lock_addr, ~0UL, 0UL, cas_buffer, mask,
    //         mask,
    //                               cxt);
    // #endif
    //     };
    ...
    //     // #ifdef TREE_ENABLE_EMBEDDING_LOCK
    //     //     // write w/o unlock
    //     //     auto write_without_unlock = [=](const GlobalAddress&
    //     //     unique_leaf_addr) {
    //     //         dsm->write_sync((const char*)leaf, unique_leaf_addr,
    //     //         sizeof(Leaf), cxt);
    //     //     };
    //     //     // write and unlock
    //     //     auto write_and_unlock = [=](const GlobalAddress&
    //     unique_leaf_addr) {
    //     //         leaf->unlock();
    //     //         dsm->write_sync((const char*)leaf, unique_leaf_addr,
    //     //         sizeof(Leaf), cxt);
    ...
    //     //     lock_handover =
    //     //         local_lock_table[dsm->getMyThreadID() /
    //     //         define::kHandOverGroupSize]
    //     //             ->acquire_local_lock(leaf_addr, &busy_waiting_queue,
    //     cxt,
    //     //             coro_id);
    //     // #endif
    //     // FIXME! no handover now
    //     //  if (lock_handover) {
    //     //      goto write_leaf;
    //     //  }
    ...
    // #if defined(USE_MCS)
    //     MCS_LOCK mcslk(GADD(leaf_addr, LeafAlign16));
    //     mcslk.lock(cxt, coro_id);
    // #else
    // re_acquire:
    //     if (!acquire_lock(leaf_addr)) {
    //         if (cxt != nullptr) {
    //             busy_waiting_queue.push(
    //                 std::make_pair(coro_id, []() { return true; }));
    ...
    //     //     local_lock_table[dsm->getMyThreadID() /
    //     define::kHandOverGroupSize]
    //     //         ->release_local_lock(leaf_addr, unlock,
    //     write_without_unlock,
    //     //                              write_and_unlock);
    //     // #else
    //     //     dsm->write_sync((const char*)leaf, leaf_addr, sizeof(Leaf),
    //     cxt);
    //     //     local_lock_table[dsm->getMyThreadID() /
    //     define::kHandOverGroupSize]
    //     //         ->release_local_lock(leaf_addr, unlock);
    ...
    //     // dsm->write_sync((const char*)leaf, leaf_addr, sizeof(Leaf), cxt);
    //     mcslk.unlock(cxt, coro_id);
    //     return;
    // #elif defined(TREE_ENABLE_EMBEDDING_LOCK)
    //     // write back the lock at the same time
    //     leaf->unlock();
    //     dsm->write_sync((const char*)leaf, leaf_addr, sizeof(Leaf), cxt);
    // #else
    ...
    //     dsm->write_cas_mask_sync(rs[0], rs[1], ~0UL, 0UL, mask, mask, cxt);
    // #endif
    //     // #endif
    //     return;
}

bool Tree::out_of_place_update_leaf(const Key& k,
                                    Value& v,
                                    int depth,
    ...
    //             local_lock_table[dsm->getMyThreadID() /
    //             define::kHandOverGroupSize]
    //                 ->acquire_local_lock(k, &busy_waiting_queue, cxt,
    //                 coro_id);
    //     }
    // #endif
    if (lock_handover) {
    ...
    if (res) {
        ...
        dsm->write(zero_byte,
                   GADD(old_e.addr(), STRUCT_OFFSET(Leaf, valid_byte)),
...
update_finish:
    ...
    //         local_lock_table[dsm->getMyThreadID() /
    //         define::kHandOverGroupSize]
    //             ->release_local_lock(k, res, old_e);
    //     }
    // #endif
    return res;
}
...
void Tree::lock_node(const GlobalAddress& node_addr,
    ...
    auto acquire_lock = [=](const GlobalAddress& unique_node_addr) {
        ...
        return dsm->cas_dm_mask_sync(lock_addr, 0UL, ~0UL, cas_buffer, mask,
                                     cxt);
    };
    ...
    //         local_lock_table[dsm->getMyThreadID() /
    //         define::kHandOverGroupSize]
    //             ->acquire_local_lock(node_addr, &busy_waiting_queue, cxt,
    //             coro_id);
    // #endif
    if (lock_handover) {
        return;
...
void Tree::unlock_node(const GlobalAddress& node_addr,
    ...
    auto unlock = [=](const GlobalAddress& unique_node_addr) {
        ...
        uint64_t mask;
        get_on_chip_lock_addr(unique_node_addr, lock_addr, mask);
        dsm->cas_dm_mask_sync(lock_addr, ~0UL, 0UL, cas_buffer, mask, mask,
                              cxt);
    };

    // #ifdef TREE_TEST_HOCL_HANDOVER
    //     local_lock_table[dsm->getMyThreadID() / define::kHandOverGroupSize]
    //         ->release_local_lock(node_addr, unlock);
    // #else
    unlock(node_addr);
    // #endif
    return;
...
bool Tree::out_of_place_write_leaf(const Key& k,
                                   Value& v,
                                   int depth,
                                   GlobalAddress& leaf_addr,
                                   uint8_t partial_key,
...
#ifdef TREE_ENABLE_WRITE_COMBINING
    if (local_lock_table[(dsm->getMyThreadID() - 1) /
                         define::kHandOverGroupSize]
            ->get_combining_value(k, v))
        unwrite = true;
#endif
    ...
    if (unwrite) {  // !ONLY allocate once
                    // reserve MCS LOCK
        GlobalAddress vaddr = dsm->alloc(8);
        uint64_t* tmp = (dsm->get_rbuf(coro_id)).get_cas_buffer();
        *tmp = v;
        dsm->write_sync((char*)tmp, vaddr, 8, cxt);
        auto leaf_buffer = (dsm->get_rbuf(coro_id)).get_leaf_buffer();
        DATA_POINTER ne;
        ne.nodeID = vaddr.nodeID, ne.offset = vaddr.offset, ne.version = 0;
        new (leaf_buffer) Leaf(k, ne.to_uint64(), e_ptr);
        constexpr uint64_t LeafAlign16 = (sizeof(Leaf) + 15) / 16 * 16;
        uint64_t* mcsMN = (uint64_t*)(leaf_buffer + LeafAlign16);
        mcsMN[0] = mcsMN[1] = 0;
        leaf_addr = dsm->alloc(LeafAlign16 + 2 * sizeof(uint64_t));
        dsm->write_sync(leaf_buffer, leaf_addr,
                        LeafAlign16 + 2 * sizeof(uint64_t), cxt);
    ...
    // cas entry
    auto new_e = InternalEntry(
        partial_key, sizeof(Leaf) < 128 ? sizeof(Leaf) : 0, leaf_addr);
    auto remote_cas = [=]() {
        return dsm->cas_sync(e_ptr, (uint64_t)old_e, (uint64_t)new_e,
                             ret_buffer, cxt);
    };
...
bool Tree::read_node(InternalEntry& p,
                     bool& type_correct,
                     char* node_buffer,
                     const GlobalAddress& p_ptr,
                     int depth,
...
bool Tree::out_of_place_write_node(const Key& k,
                                   Value& v,
                                   int depth,
                                   GlobalAddress& leaf_addr,
                                   int partial_len,
                                   uint8_t diff_partial,
                                   const GlobalAddress& e_ptr,
                                   const InternalEntry& old_e,
                                   const GlobalAddress& node_addr,
                                   uint64_t* ret_buffer,
    ...
    auto leaf_buffer = (dsm->get_rbuf(coro_id)).get_leaf_buffer();
    auto leaf_e_ptr = GADD(
        node_addrs[new_node_num - 1],
    ...
    if (leaf_unwrite) {  // !ONLY allocate once
        ...
        *tmp = v;
        ...
        DATA_POINTER ne;
        ne.nodeID = vaddr.nodeID, ne.offset = vaddr.offset, ne.version = 0;
        new (leaf_buffer) Leaf(k, ne.to_uint64(), leaf_e_ptr);
        // reserve MCS LOCK
        uint64_t* mcsMN = (uint64_t*)(leaf_buffer + LeafAlign16);
        mcsMN[0] = mcsMN[1] = 0;
        leaf_addr = dsm->alloc(LeafAlign16 + 2 * sizeof(uint64_t));
    ...
    node_pages[new_node_num - 1]->records[0] =
        InternalEntry(diff_partial, old_e);
    ...
    if (leaf_unwrite) {
        ...
        // reserve for mcs
        rs[new_node_num].size = LeafAlign16 + 2 * sizeof(uint64_t);
        rs[new_node_num].is_on_chip = false;
    }
    dsm->write_batches_sync(
        rs, (leaf_unwrite ? new_node_num + 1 : new_node_num), cxt, coro_id);
...
void Tree::cas_node_type(NodeType next_type,
                         GlobalAddress p_ptr,
                         InternalEntry p,
                         Header hdr,
                         CoroContext* cxt,
    ...
    auto remote_cas_header = [=, &hdr]() {
        return dsm->cas_mask_sync(header_addr, hdr, Header(next_type),
                                  cas_buffer_2, Header::node_type_mask,
                                  Header::node_type_mask, cxt);
    };
...
re_switch:
    ...
    if (!res.first) {
        ...
        // handle the conflict when switch & split/delete happen at the same
        // time
        while (p != InternalEntry::Null() && !p.is_leaf &&
               p.addr() != node_addr) {
            read_first_entry();
...
bool Tree::insert_behind(const Key& k,
                         Value& v,
                         int depth,
                         GlobalAddress& leaf_addr,
                         uint8_t partial_key,
    ...
    for (i = 0; i < 256 - max_num; ++i) {
        ...
        bool res = out_of_place_write_leaf(k, v, depth, leaf_addr, partial_key,
                                           e_ptr, InternalEntry::Null(),
                                           node_addr, ret_buffer, cxt, coro_id);
        ...
        // cas fail, check
            ...
            if (e.partial == partial_key) {  // same partial keys insert to the
                                             // same empty slot
                inserted_idx = slot_id;
...
bool Tree::search(const Key& k, Value& v, CoroContext* cxt, int coro_id) {
    ...
    lock_res =
        local_lock_table[(dsm->getMyThreadID() - 1) /
                         define::kHandOverGroupSize]
            ->acquire_local_read_lock(k, &busy_waiting_queue, cxt, coro_id);
...
next:
    ...
    if (p.is_leaf) {
        ...
        if (_k == k) {
            DATA_POINTER dp;
            dp.val = leaf->get_value();
            GlobalAddress vaddr{dp.nodeID, dp.offset};
            // GlobalAddress vaddr = leaf->get_value();

            uint64_t* tmp = (dsm->get_rbuf(coro_id)).get_cas_buffer();
            dsm->read_sync((char*)tmp, vaddr, 8, cxt);
            v = *tmp;
            search_res = true;
        } else {
            search_res = false;
...
void Tree::search_entries(const Key& from,
                          const Key& to,
                          int target_depth,
    ...
    // search local cache
#ifdef TREE_ENABLE_CACHE
    from_cache = index_cache->search_from_cache(from, entry_ptr_ptr, entry_ptr,
                                                entry_idx);
    if (from_cache) {  // cache hit
        assert(entry_idx >= 0);
        p_ptr = GADD(entry_ptr->addr, sizeof(InternalEntry) * entry_idx);
...
next:
    ...
    if (depth == target_depth) {
        res.push_back(ScanContext(p, p_ptr, depth - 1, from_cache,
                                  entry_ptr_ptr, entry_ptr, from, to, BORDER,
                                  BORDER));
        goto search_finish;
    }
    if (p.is_leaf) {
    ...
    if (from_cache && !type_correct) {
        index_cache->invalidate(
            entry_ptr_ptr, entry_ptr);  // invalidate the out dated node type
    }
#else
    UNUSED(type_correct);
#endif
    for (int i = 0; i < hdr.partial_len; ++i) {
        ...
        if (hdr.depth + i + 1 == target_depth) {
            range_query_on_page(p_node, from_cache, depth - 1, p_ptr, p, from,
                                to, BORDER, BORDER, res);
            goto search_finish;
        }
    }
    depth = hdr.depth + hdr.partial_len;
...
*/
// [from, to)
void Tree::range_query(const Key& from,
                       const Key& to,
...
next_level:
    ...
    for (auto& s : survivors) {
        ...
        if (tokens.find(token) == tokens.end()) {
            RdmaOpRegion r;
            r.source =
                (uint64_t)range_buffer + cnt * define::allocationPageSize;
            r.dest = p.addr();
            r.size =
                p.is_leaf
                    ? std::max((unsigned long)p.kv_len, sizeof(Leaf))
    ...
    for (int i = 0; i < cnt; ++i) {
...
#ifdef TREE_ENABLE_CACHE
                if (si[i].from_cache) {
                    index_cache->invalidate(si[i].entry_ptr_ptr,
                                            si[i].entry_ptr);
                }
#endif
                // re-read leaf entry
        ...
        // 3.2 if it is node, check & choose in-range entry in it
            ...
            range_query_on_page(node, si[i].from_cache, si[i].depth,
                                si[i].e_ptr, si[i].e, si[i].from, si[i].to,
                                si[i].l_state, si[i].r_state, survivors);
        }
    }
    goto next_level;
}

void Tree::range_query_on_page(InternalPage* page,
                               bool from_cache,
                               int depth,
                               GlobalAddress p_ptr,
                               InternalEntry p,
    ...
    for (int j = 0; j < max_num; ++j) {
        ...
        if (e_l_state != OUTSIDE && e_r_state != OUTSIDE) {
            ...
            if (e_l_state == INSIDE) {
                for (int i = 0; i < hdr.partial_len; ++i)
                    next_from =
                        remake_prefix(next_from, hdr.depth + i, hdr.partial[i]);
                next_from = remake_prefix(
                    next_from, hdr.depth + hdr.partial_len, e.partial);
            }
            if (e_r_state == INSIDE) {
                for (int i = 0; i < hdr.partial_len; ++i)
                    next_to =
                        remake_prefix(next_to, hdr.depth + i, hdr.partial[i]);
                next_to = remake_prefix(next_to, hdr.depth + hdr.partial_len,
                                        e.partial);
            }
            res.push_back(ScanContext(
                e,
                GADD(p.addr(), sizeof(GlobalAddress) + sizeof(Header) +
                                   j * sizeof(InternalEntry)),
                hdr.depth + hdr.partial_len, false, nullptr, nullptr, next_from,
                next_to, e_l_state, e_r_state));
        }
    }
}
...
void Tree::run_coroutine(GenFunc gen_func,
    ...
    for (int i = 0; i < coro_cnt; ++i) {
        ...
        worker[i] = CoroCall(
            std::bind(&Tree::coro_worker, this, _1, gen, work_func, i));
    }
...
void Tree::coro_worker(CoroYield& yield,
                       RequstGen* gen,
                       WorkFunc work_func,
                       int coro_id) {
    CoroContext ctx;
...
void Tree::clear_debug_info() {
    ...
    memset(possible_write_handover_num, 0, sizeof(uint64_t) * MAX_APP_THREAD);
    ...
    memset(read_node_type, 0,
           sizeof(uint64_t) * MAX_APP_THREAD * MAX_NODE_TYPE_NUM);
