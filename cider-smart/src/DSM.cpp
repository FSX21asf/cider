...
thread_local LocalAllocator DSM::local_allocators[MEMORY_NODE_NUM]
                                                 [NR_DIRECTORY];
...
DSM::DSM(const DSMConfig& conf)
    ...
    if (myNodeID < MEMORY_NODE_NUM) {  // start memory server
        for (int i = 0; i < NR_DIRECTORY; ++i) {
            dirAgent[i] = new Directory(dirCon[i], remoteInfo, MEMORY_NODE_NUM,
                                        i, myNodeID);
        }
...
DSM::~DSM() {
    hugePageFree((void*)baseAddr, conf.dsmSize * define::GB);
}
...
Key DSM::getNoComflictKey(uint64_t key_hash,
...
void DSM::write_batches_sync(RdmaOpRegion* rs,
                             int k,
                             CoroContext* ctx,
                             int coro_id) {
...
void DSM::write_cas_mask(RdmaOpRegion& write_ror,
                         RdmaOpRegion& cas_ror,
                         uint64_t equal,
                         uint64_t val,
    ...
    if (ctx == nullptr) {
        rdmaWriteCasMask(iCon->data[0][node_id], write_ror, cas_ror, equal, val,
                         compare_mask, swap_mask, signal);
    } else {
        rdmaWriteCasMask(iCon->data[0][node_id], write_ror, cas_ror, equal, val,
                         compare_mask, swap_mask, true, ctx->coro_id);
        (*ctx->yield)(*ctx->master);
    }
}
void DSM::write_cas_mask_sync(RdmaOpRegion& write_ror,
                              RdmaOpRegion& cas_ror,
                              uint64_t equal,
                              uint64_t val,
                              uint64_t compare_mask,
                              uint64_t swap_mask,
                              CoroContext* ctx) {
    write_cas_mask(write_ror, cas_ror, equal, val, compare_mask, swap_mask,
                   true, ctx);
    if (ctx == nullptr) {
        ibv_wc wc;
        pollWithCQ(iCon->cq, 1, &wc);
...
void DSM::read_cas(RdmaOpRegion& read_ror,
                   RdmaOpRegion& cas_ror,
                   uint64_t equal,
                   uint64_t val,
                   bool signal,
    ...
    if (ctx == nullptr) {
        rdmaReadCas(iCon->data[0][node_id], read_ror, cas_ror, equal, val,
                    signal);
    } else {
...
bool DSM::read_cas_sync(RdmaOpRegion& read_ror,
                        RdmaOpRegion& cas_ror,
                        uint64_t equal,
                        uint64_t val,
                        CoroContext* ctx) {
...
void DSM::cas_write(RdmaOpRegion& cas_ror,
    ...
    if (ctx == nullptr) {
        rdmaCasWrite(iCon->data[0][node_id], cas_ror, write_ror, equal, val,
                     signal);
    } else {
        rdmaCasWrite(iCon->data[0][node_id], cas_ror, write_ror, equal, val,
                     true, ctx->coro_id);
...
bool DSM::cas_write_sync(RdmaOpRegion& cas_ror,
                         RdmaOpRegion& write_ror,
                         uint64_t equal,
                         uint64_t val,
                         CoroContext* ctx) {
    if (GlobalAddress{cas_ror.dest}.nodeID ==
        GlobalAddress{write_ror.dest}.nodeID) {
        ...
        if (!cas_ror.is_on_chip)
            cas(GlobalAddress{cas_ror.dest}, equal, val,
                (uint64_t*)cas_ror.source, true, ctx);
        else
            cas_dm(GlobalAddress{cas_ror.dest}, equal, val,
                   (uint64_t*)cas_ror.source, true, ctx);
        if (!write_ror.is_on_chip)
            write((const char*)write_ror.source, GlobalAddress{write_ror.dest},
                  write_ror.size, true, ctx);
        else
            write_dm((const char*)write_ror.source,
                     GlobalAddress{write_ror.dest}, write_ror.size, true, ctx);
...
void DSM::two_cas_mask(RdmaOpRegion& cas_ror_1,
                       uint64_t equal_1,
                       uint64_t val_1,
                       uint64_t mask_1,
                       RdmaOpRegion& cas_ror_2,
                       uint64_t equal_2,
                       uint64_t val_2,
                       uint64_t mask_2,
                       bool signal,
    ...
    if (ctx == nullptr) {
        rdmaTwoCasMask(iCon->data[0][node_id], cas_ror_1, equal_1, val_1,
                       mask_1, cas_ror_2, equal_2, val_2, mask_2, signal);
    } else {
        rdmaTwoCasMask(iCon->data[0][node_id], cas_ror_1, equal_1, val_1,
                       mask_1, cas_ror_2, equal_2, val_2, mask_2, true,
                       ctx->coro_id);
        (*ctx->yield)(*ctx->master);
    }
}
std::pair<bool, bool> DSM::two_cas_mask_sync(RdmaOpRegion& cas_ror_1,
                                             uint64_t equal_1,
                                             uint64_t val_1,
                                             uint64_t mask_1,
                                             RdmaOpRegion& cas_ror_2,
                                             ...
                                             uint64_t mask_2,
                                             CoroContext* ctx) {
    if (GlobalAddress{cas_ror_1.dest}.nodeID ==
        GlobalAddress{cas_ror_2.dest}.nodeID) {
        two_cas_mask(cas_ror_1, equal_1, val_1, mask_1, cas_ror_2, equal_2,
                     val_2, mask_2, true, ctx);
        ...
        if (!cas_ror_1.is_on_chip)
            cas_mask(GlobalAddress{cas_ror_1.dest}, equal_1, val_1,
                     (uint64_t*)cas_ror_1.source, mask_1, mask_1, true, ctx);
        else
            cas_dm_mask(GlobalAddress{cas_ror_1.dest}, equal_1, val_1,
                        (uint64_t*)cas_ror_1.source, mask_1, mask_1, true, ctx);
        if (!cas_ror_2.is_on_chip)
            cas_mask(GlobalAddress{cas_ror_2.dest}, equal_2, val_2,
                     (uint64_t*)cas_ror_2.source, mask_2, mask_2, true, ctx);
        else
            cas_dm_mask(GlobalAddress{cas_ror_2.dest}, equal_2, val_2,
                        (uint64_t*)cas_ror_2.source, mask_2, mask_2, true, ctx);
    ...
    return std::make_pair(equal_1 == *(uint64_t*)cas_ror_1.source,
                          equal_2 == *(uint64_t*)cas_ror_2.source);
}
...
void DSM::cas_mask(GlobalAddress gaddr,
    ...
    if (ctx == nullptr) {
        rdmaCompareAndSwapMask(
            iCon->data[0][gaddr.nodeID], (uint64_t)rdma_buffer,
            remoteInfo[gaddr.nodeID].dsmBase + gaddr.offset, equal, val,
            iCon->cacheLKey, remoteInfo[gaddr.nodeID].dsmRKey[0], compare_mask,
            swap_mask, signal);
    } else {
        rdmaCompareAndSwapMask(
            iCon->data[0][gaddr.nodeID], (uint64_t)rdma_buffer,
...
bool DSM::cas_mask_sync(GlobalAddress gaddr,
                        ...
                        uint64_t swap_mask,
                        CoroContext* ctx) {
    cas_mask(gaddr, equal, val, rdma_buffer, compare_mask, swap_mask, true,
             ctx);
...
void DSM::faa_boundary(GlobalAddress gaddr,
    ...
    if (ctx == nullptr) {
        rdmaFetchAndAddBoundary(
            ...
            iCon->cacheLKey, remoteInfo[gaddr.nodeID].dsmRKey[0], mask, signal);
    } else {
        rdmaFetchAndAddBoundary(
            iCon->data[0][gaddr.nodeID], (uint64_t)rdma_buffer,
            remoteInfo[gaddr.nodeID].dsmBase + gaddr.offset, add_val,
            iCon->cacheLKey, remoteInfo[gaddr.nodeID].dsmRKey[0], mask, true,
            ctx->coro_id);
        (*ctx->yield)(*ctx->master);
    }
}
void DSM::faa_field_boundary_sync(GlobalAddress gaddr,
                                  uint64_t add_val,
                                  uint64_t* rdma_buffer,
                                  uint64_t mask,
                                  CoroContext* ctx) {
    faa_field_boundary(gaddr, add_val, rdma_buffer, mask, true, ctx);
    if (ctx == nullptr) {
        ibv_wc wc;
        pollWithCQ(iCon->cq, 1, &wc);
    }
}
void DSM::faa_field_boundary(GlobalAddress gaddr,
                             uint64_t add_val,
                             uint64_t* rdma_buffer,
                             uint64_t mask,
                             bool signal,
                             CoroContext* ctx) {
    if (ctx == nullptr) {
        rdmaFetchAndAddWithFieldBoundary(
            iCon->data[0][gaddr.nodeID], (uint64_t)rdma_buffer,
            remoteInfo[gaddr.nodeID].dsmBase + gaddr.offset, add_val,
            iCon->cacheLKey, remoteInfo[gaddr.nodeID].dsmRKey[0], mask, signal);
    } else {
...
void DSM::faa128_boundray(GlobalAddress gaddr,
                          uint64_t* add_val,
                          uint64_t* rdma_buffer,
                          uint64_t* mask,
                          bool signal,
                          CoroContext* ctx) {
    if (ctx == nullptr) {
        rdmaEXTfaaB(iCon->data[0][gaddr.nodeID], (uint64_t)rdma_buffer,
                    remoteInfo[gaddr.nodeID].dsmBase + gaddr.offset,
                    (uint64_t)add_val, iCon->cacheLKey,
                    remoteInfo[gaddr.nodeID].dsmRKey[0], (uint64_t)mask, 4,
                    signal);
    } else {
        rdmaEXTfaaB(iCon->data[0][gaddr.nodeID], (uint64_t)rdma_buffer,
                    remoteInfo[gaddr.nodeID].dsmBase + gaddr.offset,
...
void DSM::faa128_boundray_sync(GlobalAddress gaddr,
                               uint64_t* add_val,
                               uint64_t* rdma_buffer,
                               uint64_t* mask,
                               CoroContext* ctx) {
    faa128_boundray(gaddr, add_val, rdma_buffer, mask, true, ctx);
    if (ctx == nullptr) {
        ibv_wc wc;
        pollWithCQ(iCon->cq, 1, &wc);
    }
    // FIXME:128 must bswap
    rdma_buffer[0] = __bswap_64(rdma_buffer[0]);
    rdma_buffer[1] = __bswap_64(rdma_buffer[1]);
}
void DSM::cas128_mask(GlobalAddress gaddr,
                      uint64_t* equal,
                      uint64_t* val,
                      uint64_t* rdma_buffer,
                      uint64_t* compare_mask,
                      ...
                      CoroContext* ctx) {
    if (ctx == nullptr) {
        rdmaEXTcasm(iCon->data[0][gaddr.nodeID], (uint64_t)rdma_buffer,
                    remoteInfo[gaddr.nodeID].dsmBase + gaddr.offset,
                    (uint64_t)equal, (uint64_t)val, iCon->cacheLKey,
                    remoteInfo[gaddr.nodeID].dsmRKey[0], (uint64_t)compare_mask,
                    (uint64_t)swap_mask, 4, signal);
    } else {
        rdmaEXTcasm(iCon->data[0][gaddr.nodeID], (uint64_t)rdma_buffer,
                    remoteInfo[gaddr.nodeID].dsmBase + gaddr.offset,
                    (uint64_t)equal, (uint64_t)val, iCon->cacheLKey,
                    remoteInfo[gaddr.nodeID].dsmRKey[0], (uint64_t)compare_mask,
                    (uint64_t)swap_mask, 4, true, ctx->coro_id);
        (*ctx->yield)(*ctx->master);
    }
}

bool DSM::cas128_mask_sync(GlobalAddress gaddr,
                           uint64_t* equal,
                           uint64_t* val,
                           uint64_t* rdma_buffer,
                           uint64_t* compare_mask,
                           uint64_t* swap_mask,
                           CoroContext* ctx) {
    cas128_mask(gaddr, equal, val, rdma_buffer, compare_mask, swap_mask, true,
                ctx);
    ...
    rdma_buffer[0] = __bswap_64(rdma_buffer[0]);
    rdma_buffer[1] = __bswap_64(rdma_buffer[1]);
    return ((equal[0] & compare_mask[0]) ==
            (rdma_buffer[0] & compare_mask[0])) &&
           ((equal[1] & compare_mask[1]) == (rdma_buffer[1] & compare_mask[1]));
}
...
void DSM::cas_dm_mask(GlobalAddress gaddr,
    ...
    if (ctx == nullptr) {
        rdmaCompareAndSwapMask(
            iCon->data[0][gaddr.nodeID], (uint64_t)rdma_buffer,
            remoteInfo[gaddr.nodeID].lockBase + gaddr.offset, equal, val,
            iCon->cacheLKey, remoteInfo[gaddr.nodeID].lockRKey[0], compare_mask,
            swap_mask, signal);
    } else {
        rdmaCompareAndSwapMask(
            iCon->data[0][gaddr.nodeID], (uint64_t)rdma_buffer,
...
bool DSM::cas_dm_mask_sync(GlobalAddress gaddr,
                           ...
                           uint64_t swap_mask,
                           CoroContext* ctx) {
    cas_dm_mask(gaddr, equal, val, rdma_buffer, compare_mask, swap_mask, true,
                ctx);
...
void DSM::faa_dm_boundary(GlobalAddress gaddr,
    ...
    if (ctx == nullptr) {
        rdmaFetchAndAddBoundary(
            iCon->data[0][gaddr.nodeID], (uint64_t)rdma_buffer,
            remoteInfo[gaddr.nodeID].lockBase + gaddr.offset, add_val,
            iCon->cacheLKey, remoteInfo[gaddr.nodeID].lockRKey[0], mask,
            signal);
    } else {
        rdmaFetchAndAddBoundary(
            iCon->data[0][gaddr.nodeID], (uint64_t)rdma_buffer,
            remoteInfo[gaddr.nodeID].lockBase + gaddr.offset, add_val,
            iCon->cacheLKey, remoteInfo[gaddr.nodeID].lockRKey[0], mask, true,
            ctx->coro_id);
        (*ctx->yield)(*ctx->master);
    }
}
