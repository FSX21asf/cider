...
inline bool operator<(const timespec& a, const timespec& b) {
    return std::make_pair(a.tv_sec, a.tv_nsec) <
           std::make_pair(b.tv_sec, b.tv_nsec);
}
inline bool operator>=(const timespec& a, const timespec& b) {
    return !(a < b);
}
...
inline uint64_t to_offset(uint64_t k) {
    return ((CityHash64((char*)&k, sizeof(k))) % addressCnt) *
           (msgSize + 8);  // kv + lock
}
...
void thread_run(int id) {
    ...
    while (!need_stop) {
        ...
        // timer
        if (is_read)
            do_read(dsm, addr, s, e);
        else
            cas_cnt = do_write(dsm, addr, s, e);
        auto& logs = is_read ? read_logs[id] : write_logs[id];
        if (log_start)
            logs.push_back(Record(s, e, cas_cnt, addr));
    }
...
int main(int argc, char* argv[]) {
    ...
    auto sort_logs = [=](std::vector<Record>& total_log,
                         std::vector<Record>* logs) {
        for (int i = 0; i < MAX_APP_THREAD; ++i) {
            total_log.insert(total_log.end(), logs[i].begin(), logs[i].end());
        }
        std::sort(total_log.begin(), total_log.end(),
                  [](const Record& a, const Record& b) { return a.s < b.s; });
    };
    ...
    auto calculate_redundant = [=](const std::vector<Record>& total_log) {
        ...
        while (i < (int)total_log.size()) {
            ...
            time_windows += (r1.e.tv_sec - r1.s.tv_sec) * 1000000 +
                            (double)(r1.e.tv_nsec - r1.s.tv_nsec) / 1000;
            ++windows_cnt;
        }
        return CalResult{(double)redundant_op / windows_cnt,
                         (double)redundant_cas / windows_cnt,
                         time_windows / windows_cnt};
    };
