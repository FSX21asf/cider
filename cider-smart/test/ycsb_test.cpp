   ...
   public:
    ...
    Request next() override {
        ...
        if (req[cur].is_insert) {
            ...
            if (kIsStr) {
                req[cur].k =
                    req[cur].k + epoch_id;  // For insert workloads, key should
                                            // remain nonexist
            } else if (flag) {
        ...
        while (trans_in >> op >> int_k) {
            ...
            if (rm_write_conflict) {
                if (r.is_update || r.is_insert) {
                    uint64_t all_thread_num =
                        kThreadCount * dsm->getClusterSize();
                    r.k = dsm->getNoComflictKey(key_hash(r.k), my_id,
                                                all_thread_num);
                }
            }
            req[req_num++] = r;
...
void parse_args(int argc, char* argv[]) {
    if (argc != 6 && argc != 7) {
        printf(
            ...
            "[fix_range_size/rm_write_conflict]\n");
        exit(-1);
    }
    ...
    ycsb_load_path = workload_dir + "/load_" + std::string(argv[4]) +
    ...
    if (argc == 7) {
        if (kIsScan)
            ...
            rm_write_conflict = (atoi(argv[6]) != 0);
    }
...
int main(int argc, char* argv[]) {
    ...
    while (!need_stop) {
        ...
        uint64_t cluster_tp = dsm->sum(
            (uint64_t)(per_node_tp * 1000));  // only node 0 return the sum
