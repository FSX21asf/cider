...
class InlineSkipList {
   ...
   public:
    ...
    int GetHeightSum() const { return height_sum.load(); }
    ...
    if (UseCAS) {
        for (int i = 0; i < height; ++i) {
            while (true) {
                ...
                // unlikely to be helpful to try to use a different level as we
                // redo the search, because it should be unlikely that lots of
                // nodes have been inserted between prev[i] and next[i]. No
                // point in using next[i] as the after hint, because we know it
                // is stale.
                FindSpliceForLevel<false>(key_decoded, splice->prev_[i],
                                          nullptr, i, &splice->prev_[i],
                                          &splice->next_[i]);
