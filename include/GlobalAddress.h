...
class GlobalAddress {
   public:
    ...
    static GlobalAddress Max() { return GlobalAddress(MEMORY_NODE_NUM, 0); };
    ...
    static GlobalAddress Widest() { return GlobalAddress(~0ULL); };
...
inline bool operator<(const GlobalAddress& lhs, const GlobalAddress& rhs) {
    return std::make_pair(lhs.nodeID, lhs.offset) <
           std::make_pair(rhs.nodeID, rhs.offset);
}
