...
class TreeCache {
   public:
    ...
    const TreeCacheEntry* search_from_cache(const Key& k,
                                            GlobalAddress& addr,
                                            GlobalAddress& sibling_addr,
                                            uint16_t& level);
    const TreeCacheEntry* search_ptr_from_cache(const Key& k,
                                                GlobalAddress& addr,
   ...
   private:
    ...
    const TreeCacheEntry* seek_entry(
        const Key& k);  // try to return an non-nullptr cache_entry
...
inline TreeCache::TreeCache(int cache_size, DSM* dsm)
...
// [from, to）
inline bool TreeCache::add_entry(const Key& from,
                                 const Key& to,
                                 InternalNode* ptr) {
    // TODO: memory leak
...
inline const TreeCacheEntry* TreeCache::find_entry(const Key& from,
                                                   const Key& to) {
...
inline const TreeCacheEntry* TreeCache::seek_entry(const Key& from,
                                                   const Key& to) {
    TreeCacheSkipList::Iterator iter(skiplist);
    ...
    if (this->add_entry(lowest, highest, new_page)) {
        ...
        if (e && e->from == lowest && e->to == highest - 1) {
            ...
            if (ret_val == ptr) {  // cas success
                if (ret_val == nullptr) {
                    auto v =
                        free_size.fetch_add(-new_page->consumed_cache_size());
                    if (v < 0) {
                        evict();
                    }
                }
...
inline const TreeCacheEntry* TreeCache::search_from_cache(
    const Key& k,
    GlobalAddress& addr,
    GlobalAddress& sibling_addr,
    uint16_t& level) {
    ...
    auto entry =
        seek_entry(k);  // seek the lowest cached internal node surrounding k
#else
...
inline const TreeCacheEntry* TreeCache::search_ptr_from_cache(
    const Key& k,
    GlobalAddress& addr,
    const uint16_t& level) {  // get the addr of the internal node with a target
                              // level, surrounding k
...
inline void TreeCache::search_range_from_cache(
    const Key& from,
    const Key& to,
    std::vector<InternalNode>& result) {
    TreeCacheSkipList::Iterator iter(skiplist);
    ...
    while (iter.Valid()) {
        ...
        if (entry->ptr) {
            ...
            if (entry->ptr->metadata.level == 1)
                result.push_back(*(entry->ptr));  // filter: level == 1
        }
        iter.Next();
    }
}
...
inline void TreeCache::statistics() {
    printf(
        " ----- [TreeCache]:  cache size=%lu MB free_size=%.3lf MB "
        "skiplist_node_cnt=%d ----- \n",
        cache_size, (double)free_size.load() / define::MB,
        (int)skiplist_node_cnt.load());
