...
inline static TreeCacheEntry Decode(const char* val) {
    return *(TreeCacheEntry*)val;
}
...
struct TreeCacheEntryComparator {
    ...
    int cmp(const DecodedType a_v, const DecodedType b_v)
        const {  // entry larger => (to larger, from smaller)
        if (a_v.to < b_v.to) {
            return -1;
        }
