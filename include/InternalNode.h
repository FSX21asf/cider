...
class InternalEntry {
   ...
   public:
    ...
    void update(const Key& k, const GlobalAddress& ptr) {
        key = k, this->ptr = ptr;
    }
...
class InternalNode {  // must be cacheline-align
   ...
   public:
    ...
    bool is_root() const { return metadata.fence_keys == FenceKeys::Widest(); }
    ...
    int64_t consumed_cache_size() const {
        ...
        for (const auto& e : records)
            if (e.key != define::kkeyNull)
                ++cnt;
        return sizeof(uint64_t) + sizeof(InternalMetadata) +
               cnt * sizeof(InternalEntry);
    }

    static const bool IS_LEAF = false;
} __attribute__((packed));

static_assert(sizeof(InternalNode) ==
              sizeof(InternalMetadata) +
                  sizeof(InternalEntry) * define::internalSpanSize);
