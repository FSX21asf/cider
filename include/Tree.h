...
enum { FIRST_TRY, INVALID_LEAF, INVALID_NODE, FIND_NEXT };
...
class RootEntry {
   public:
    ...
    RootEntry(const uint16_t level, const GlobalAddress& ptr)
        : level(level), ptr(ptr) {}

    operator uint64_t() const { return ((uint64_t)ptr << 16) | level; }
    operator std::pair<uint16_t, GlobalAddress>() const {
        return std::make_pair(level, (GlobalAddress)ptr);
    }
...
class Tree {
   public:
    ...
    void run_coroutine(GenFunc gen_func,
                       WorkFunc work_func,
                       int coro_cnt,
                       Request* req = nullptr,
                       int req_num = 0);
   ...
   private:
    ...
    static uint64_t get_mcs_lock_info(bool is_leaf);
    void lock_node(const GlobalAddress& node_addr,
                   uint64_t* lock_buffer,
                   bool is_leaf,
                   CoroPull* sink);
    void unlock_node(const GlobalAddress& node_addr,
                     uint64_t* lock_buffer,
                     bool is_leaf,
                     CoroPull* sink,
                     bool async = false);
    ...
    // search
    bool leaf_node_search(const GlobalAddress& node_addr,
                          const GlobalAddress& sibling_addr,
                          const Key& k,
                          Value& v,
                          bool from_cache,
                          CoroPull* sink);
    bool internal_node_search(GlobalAddress& node_addr,
    ...
    // insert
    bool leaf_node_insert(const GlobalAddress& node_addr,
                          const GlobalAddress& sibling_addr,
                          const Key& k,
                          Value v,
                          bool from_cache,
                          CoroPull* sink);
    bool internal_node_insert(const GlobalAddress& node_addr,
                              const Key& k,
    ...
    // update
    bool leaf_node_update(const GlobalAddress& node_addr,
                          const GlobalAddress& sibling_addr,
                          const Key& k,
                          Value v,
                          bool from_cache,
    ...
    bool hopscotch_insert_and_unlock(LeafNode* leaf,
                                     const Key& k,
                                     Value v,
    ...
    void hopscotch_split_and_unlock(LeafNode* leaf,
                                    const Key& k,
                                    Value v,
                                    const GlobalAddress& node_addr,
                                    uint64_t* lock_buffer,
...
#endif
                                    CoroPull* sink);
    void hopscotch_search(const GlobalAddress& node_addr,
                          int hash_idx,
    ...
    bool speculative_read(const GlobalAddress& leaf_addr,
    ...
    void leaf_entry_read(const GlobalAddress& leaf_addr,
                         const int idx,
                         char* raw_leaf_buffer,
                         char* leaf_buffer,
                         CoroPull* sink,
    ...
    void entry_write_and_unlock(NODE* node,
                                const int idx,
                                const Key& k,
                                VAL v,
                                const GlobalAddress& node_addr,
    ...
    void node_write_and_unlock(NODE* node,
                               const GlobalAddress& node_addr,
                               uint64_t* lock_buffer,
                               CoroPull* sink,
                               bool async = false);
    void segment_write_and_unlock(LeafNode* leaf,
                                  int l_idx,
                                  int r_idx,
    ...
    template <class NODE,
              ...
              int ALLOC_SIZE,
              int TRANS_SIZE>
    void node_split_and_unlock(NODE* node,
                               const Key& k,
                               VAL v,
                               const GlobalAddress& node_addr,
                               uint64_t* lock_buffer,
