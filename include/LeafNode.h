...
class LeafMetadata {
   ...
   public:
    LeafMetadata()
        : h_version(), level(0), valid(1), sibling_ptr(), fence_keys() {}
    LeafMetadata(PackedVersion h_version,
                 uint8_t level,
                 uint8_t valid,
                 GlobalAddress sibling_ptr,
                 FenceKeys fence_keys)
...
class LeafEntry {
   public:
    ...
    Key key;
    union {
        Value value;  // !!!NOTE: with ENABLE_VAR_LEN_KV turned on, the value in
                      // this field will be casted into a DataPointer
   ...
   public:
    ...
    LeafEntry()
        : h_version(),
          hop_bitmap(0),
          key(define::kkeyNull),
          value(define::kValueNull) {}
    LeafEntry(const Key& k, const Value& v)
        : h_version(), hop_bitmap(0), key(k), value(v) {}
#else
    LeafEntry()
        : h_version(), key(define::kkeyNull), value(define::kValueNull) {}
    ...
    void set_hop_bit(int idx) {
        assert(idx >= 0 && idx < (int)define::neighborSize &&
...
class LeafNode {  // must be cacheline-align
   public:
    // cacheline-versions will be embedded from here with an 64-byte offset (can
    // be skipped if obj version is here)
...
static_assert(sizeof(LeafNode) ==
              sizeof(LeafMetadata) + sizeof(LeafEntry) * define::leafSpanSize);
...
class ScatteredMetadata {
   public:
    ...
    ScatteredMetadata(const LeafMetadata& metadata)
        : h_version(metadata.h_version),
          valid(1),
          sibling_ptr(metadata.sibling_ptr) {}
} __attribute__((packed));

static_assert(sizeof(ScatteredMetadata) == define::scatterMetadataSize);

inline bool operator==(const ScatteredMetadata& lhs,
                       const ScatteredMetadata& rhs) {
   ...
   public:
    ...
    ScatteredMetadata(const LeafMetadata& metadata)
        : h_version(metadata.h_version),
          valid(1),
          sibling_ptr(metadata.sibling_ptr),
          fence_keys(metadata.fence_keys) {}
} __attribute__((packed));
...
static_assert(sizeof(LeafEntryGroup) ==
              sizeof(ScatteredMetadata) +
                  sizeof(LeafEntry) * define::neighborSize);

/* Scattered Leaf Node: [lock, leaf metadata, [scattered metadata, KV, KV...] *
 * n] */
...
static_assert(sizeof(ScatteredLeafNode) ==
              sizeof(LeafEntryGroup) * define::entryGroupNum);
