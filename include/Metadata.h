...
class PackedGAddr {  // 48-bit, used by node addr/leaf addr (not entry addr)
   public:
    ...
    PackedGAddr(const GlobalAddress& addr)
        : mn_id(addr.nodeID), offset(addr.offset >> PACKED_ADDR_ALIGN_BIT) {}
    operator GlobalAddress() const {
        return GlobalAddress{mn_id, offset << PACKED_ADDR_ALIGN_BIT};
    }

    PackedGAddr(uint64_t val)
        : mn_id(val & ((1UL << define::mnIdBit) - 1)),
          offset(val >> define::mnIdBit) {}
...
class VALOCK {
   ...
   public:
    VALOCK(uint64_t vacancy_bitmap, uint64_t max_key_idx)
        : vacancy_bitmap(vacancy_bitmap),
          max_key_idx(max_key_idx),
          lock_bit(0) {}

    void update_vacancy(int l,
                        int r,
                        const std::vector<int>& empty_idxes,
                        bool is_leaf = true) {  // [l, r]
        int span_size =
            is_leaf ? define::leafSpanSize : define::internalSpanSize;
        int l_bit = find_bucket(l, span_size),
            r_bit = find_bucket(r, span_size);

        if (l_bit <= r_bit)
            for (int i = l_bit; i <= r_bit; ++i)
        ...
        for (int empty_idx : empty_idxes) {
            ...
            if (l_bit <= r_bit)
                assert(i >= l_bit && i <= r_bit);
            else
                assert((i >= 0 && i <= r_bit) ||
                       (i >= l_bit && i < (int)define::vacancyMapBit));
            vacancy_bitmap &= ~(1ULL << i);
        }
    }
    ...
    int get_read_entry_num_from_bitmap(int start_idx, bool is_leaf) {
        ...
        for (int i = 0; i < (int)define::vacancyMapBit; ++i) {
            ...
            if (!(vacancy_bitmap & (1ULL << e_bit))) {  // empty
                auto [l_idx, r_idx] = get_idx_range_in_bucket(
                    e_bit, span_size);  // [l_idx, r_idx]
                if (start_idx > l_idx)
                    continue;  // !!Important
    ...
    int get_max_key_idx() { return max_key_idx; }

   private:
    int find_bucket(int kv_idx,
                    int span_size) {  // 0~span_size => 0~bitmap_size
        double avg_per_bucket =
            static_cast<double>(span_size) / define::vacancyMapBit;
        int bucket = static_cast<int>(floor(kv_idx / avg_per_bucket));
        return bucket % define::vacancyMapBit;
    }

    std::pair<int, int> get_idx_range_in_bucket(
        int bucket,
        int span_size) {  // 0~bitmap_size => 0~span_size
        auto get_first = [=](int bucket_id) {
            double avg_per_bucket =
                static_cast<double>(span_size) / define::vacancyMapBit;
            int idx = bucket_id
                          ? static_cast<int>(floor(bucket_id * avg_per_bucket))
                          : 0;
...
class DataPointer {
   public:
    ...
    DataPointer(const uint64_t data_len, const GlobalAddress& ptr)
        : data_len(data_len), ptr(ptr) {}

    operator uint64_t() const { return ((uint64_t)ptr << 16) | data_len; }
    operator std::pair<uint64_t, GlobalAddress>() const {
        return std::make_pair(data_len, (GlobalAddress)ptr);
    }
} __attribute__((packed));

static_assert(sizeof(DataPointer) == 8);

class DataBlock {  // For brievity, we assume the whole key can be stored
                   // inline, and only changing the value size for evaluation
   public:
    ...
    DataBlock(const Value value)
        : rest_of_key_len(0), value_len(define::indirectValLen), value(value) {}
} __attribute__((packed));
...
struct FenceKeys {
    ...
    FenceKeys() : lowest{} {
        lowest = lowest + define::kKeyMin;
        highest.fill(0xff);
    }
    FenceKeys(const Key& lowest, const Key& highest)
        : lowest(lowest), highest(highest) {}

    static FenceKeys Widest() { return FenceKeys(); };
} __attribute__((packed));
...
inline bool operator==(const PackedVersion& lhs, const PackedVersion& rhs) {
    return (lhs.entry_version == rhs.entry_version) &&
           (lhs.node_version == rhs.node_version);
}
