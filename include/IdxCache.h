...
struct IdxCacheEntry {
    ...
    IdxCacheEntry(const GlobalAddress& leaf_addr, int kv_idx, const Key& k)
        : leaf_addr(leaf_addr),
          kv_idx(kv_idx),
          fingerprint(key2fp(k)),
          cache_entry_freq(1) {}
} __attribute__((packed));

static_assert(sizeof(IdxCacheEntry) ==
              sizeof(GlobalAddress) + sizeof(uint16_t) * 2 + sizeof(int32_t));
...
class IdxCache {
   public:
    ...
    bool search_idx_from_cache(const GlobalAddress& leaf_addr,
                               int l_idx,
                               int r_idx,
                               const Key& k,
                               int& kv_idx);
   ...
   private:
    ...
    static const int BUCKET_SIZE =
        std::max(HASH_BUCKET_SIZE,
                 (int)ROUND_UP(define::kHotspotBufSize* define::MB /
                                   sizeof(IdxCacheEntry*) / HASH_TABLE_SIZE,
                               3));
...
inline IdxCache::IdxCache(int cache_size, DSM* dsm)
    : cache_size(cache_size), dsm(dsm) {
    memset(hash_table, 0,
           sizeof(IdxCacheEntry*) * HASH_TABLE_SIZE * BUCKET_SIZE);
    free_size.store(define::MB * cache_size);
    delay_cnt.store(0);
    is_disabled = (cache_size == 0);
}

inline bool IdxCache::add_to_cache(const GlobalAddress& leaf_addr,
                                   int kv_idx,
                                   const Key& k) {
    if (is_disabled) {
        return false;
    ...
    for (const auto& e : bucket)
        if (e && e->leaf_addr == leaf_addr && e->kv_idx == kv_idx) {
            if (e->fingerprint != key2fp(k)) {
                __sync_bool_compare_and_swap(&(e->fingerprint), e->fingerprint,
                                             key2fp(k));
                __sync_bool_compare_and_swap(&(e->cache_entry_freq),
    ...
    for (int i = 0; i < BUCKET_SIZE; ++i) {
        ...
        if (!e) {
            ...
            delete new_entry;
            return false;
        } else if (e->cache_entry_freq < min_freq)
            min_idx = i, min_freq = e->cache_entry_freq;
    }
    // evict the lfu one
...
inline bool IdxCache::search_idx_from_cache(const GlobalAddress& leaf_addr,
                                            int l_idx,
                                            int r_idx,
                                            const Key& k,
                                            int& kv_idx) {
    ...
    auto search_for_candidate = [&](int idx) {
        ...
        auto& bucket = hash_table[ht_idx];
        for (const auto& e : bucket)
            if (e && e->leaf_addr == leaf_addr && e->kv_idx == idx) {
...
inline void IdxCache::evict_one() {
    ...
    auto find_lfu_idx = [](IdxCacheEntry** bucket, int& min_idx,
                           int& min_freq) {
        for (int i = 0; i < BUCKET_SIZE; ++i) {
            const auto& e = bucket[i];
            if (e && e->cache_entry_freq < min_freq)
                min_idx = i, min_freq = e->cache_entry_freq;
        }
    };
    ...
    // erase an entry
    auto& entry =
        (min_freq_1 < min_freq_2 ? bucket_1[min_idx_1] : bucket_2[min_idx_2]);
