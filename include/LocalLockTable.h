...
class LocalLockTable {
   public:
    ...
    // read-delegation
    std::pair<bool, bool> acquire_local_read_lock(
        const Key& k,
    ...
    // write-combining
    std::pair<bool, bool> acquire_local_write_lock(
        const Key& k,
        const Value& v,
        CoroQueue* waiting_queue = nullptr,
        CoroPull* sink = nullptr);
    ...
    bool acquire_local_read_lock(const GlobalAddress& addr,
                                 CoroQueue* waiting_queue = nullptr,
                                 CoroPull* sink = nullptr);
    void release_local_read_lock(const GlobalAddress& addr,
                                 bool& res,
                                 Value& ret_value);
...
// read-delegation
inline std::pair<bool, bool> LocalLockTable::acquire_local_read_lock(
    const Key& k,
...
// read-delegation
inline void LocalLockTable::release_local_read_lock(
    const Key& k,
    std::pair<bool, bool> acquire_ret,
    bool& res,
    Value& ret_value) {
...
// write-combining
inline std::pair<bool, bool> LocalLockTable::acquire_local_write_lock(
    const Key& k,
    const Value& v,
    CoroQueue* waiting_queue,
    CoroPull* sink) {
...
// write-combining
inline void LocalLockTable::release_local_write_lock(
    const Key& k,
    std::pair<bool, bool> acquire_ret) {
    if (acquire_ret.second)
        return;
...
// cas-handover
inline void LocalLockTable::release_local_lock(const Key& k,
                                               bool& res,
                                               InternalEntry& ret_p) {
    auto& node = local_locks[get_hashed_local_lock_index(k)];
