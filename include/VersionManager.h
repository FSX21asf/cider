...
class VersionManager {
   public:
    ...
    static bool decode_header_versions(char* input_buffer,
                                       char* output_buffer,
    ...
    static void encode_entry_versions(char* input_buffer,
                                      char* output_buffer,
                                      uint64_t first_offset);
   ...
   private:
    static const int FIRST_OFFSET = std::min(
        (uint64_t)define::cachelineSize,
        sizeof(NODE));  // the address of the first encoded cacheline version
};

/* Call after read the entire node. Both entry- and node-level consistency are
 * checked. */
template <class NODE, class ENTRY>
inline bool VersionManager<NODE, ENTRY>::decode_node_versions(
    char* input_buffer,
    char* output_buffer) {
    ...
    for (uint64_t i = FIRST_OFFSET, j = FIRST_OFFSET; j < sizeof(NODE);
        ...
        memcpy(output_buffer + j, input_buffer + i,
               std::min((size_t)define::blockSize, sizeof(NODE) - j));
        PackedVersion obj_version;
        if (j < STRUCT_OFFSET(NODE, records)) {
            obj_version = metadata.h_version;
        } else {
            obj_version =
                entries[(j - STRUCT_OFFSET(NODE, records)) / sizeof(ENTRY)]
                    .h_version;
        }
    ...
    for (const auto& entry : entries)
        is_consistent &= (node_version == entry.h_version.node_version);
 ...
 * incremented. */
template <class NODE, class ENTRY>
inline void VersionManager<NODE, ENTRY>::encode_node_versions(
    char* input_buffer,
    char* output_buffer) {
    ...
    memcpy(output_buffer, input_buffer, FIRST_OFFSET);
    for (uint64_t i = FIRST_OFFSET, j = FIRST_OFFSET; i < sizeof(NODE);
         i += define::blockSize, j += define::blockSize) {
        PackedVersion obj_version;
        if (i < STRUCT_OFFSET(NODE, records)) {
            ...
            obj_version =
                entries[(i - STRUCT_OFFSET(NODE, records)) / sizeof(ENTRY)]
                    .h_version;
        }
        memcpy(output_buffer + j, &obj_version, sizeof(PackedVersion));
        j += sizeof(PackedVersion);
        memcpy(output_buffer + j, input_buffer + i,
               std::min((size_t)define::blockSize, sizeof(NODE) - i));
    }
}

template <class NODE, class ENTRY>
inline std::tuple<uint64_t, uint64_t, uint64_t>
VersionManager<NODE, ENTRY>::get_offset_info(
    int start_entry_idx,
    ...
    if (dist < 0) {
        ...
        first_offset = std::min((uint64_t)FIRST_OFFSET - decoded_start_offset,
                                sizeof(ENTRY) * entry_num);
    } else {
        auto version_cnt =
            dist / define::blockSize + (dist % define::blockSize ? 1 : 0);
        raw_offset = decoded_start_offset + version_cnt * define::versionSize;
        first_offset =
            std::min((uint64_t)FIRST_OFFSET + version_cnt * define::blockSize -
...
/* Call before write the entire entry. Only the entry_versions will be
 * incremented. */
template <class NODE, class ENTRY>
inline void VersionManager<NODE, ENTRY>::encode_entry_versions(
    char* input_buffer,
    char* output_buffer,
    uint64_t first_offset) {
    ...
    // generate cacheline versions
    memcpy(output_buffer, input_buffer, first_offset);
    for (uint64_t i = first_offset, j = first_offset; i < sizeof(ENTRY);
         i += define::blockSize, j += define::blockSize) {
        memcpy(output_buffer + j, (void*)&obj_version, sizeof(PackedVersion));
        j += sizeof(PackedVersion);
        memcpy(output_buffer + j, input_buffer + i,
               std::min((size_t)define::blockSize, sizeof(ENTRY) - i));
    }
}
...
template <class NODE, class ENTRY>
inline void VersionManager<NODE, ENTRY>::encode_segment_versions(
    char* input_buffer,
    char* output_buffer,
    uint64_t first_offset,
    const std::vector<int>& hopped_idxes,
...
template <class NODE, class ENTRY>
inline bool VersionManager<NODE, ENTRY>::decode_header_versions(
    char* input_buffer,
    char* output_buffer,
    uint8_t& node_version) {
    memcpy(output_buffer, input_buffer, FIRST_OFFSET);
    ...
    for (uint64_t i = FIRST_OFFSET, j = FIRST_OFFSET; j < header_size;
         i += define::blockSize, j += define::blockSize) {
        auto cacheline_version = *(PackedVersion*)(input_buffer + i);
        i += sizeof(PackedVersion);
        memcpy(output_buffer + j, input_buffer + i,
               std::min((size_t)define::blockSize, header_size - j));
...
template <class NODE, class ENTRY>
inline bool VersionManager<NODE, ENTRY>::decode_segment_versions(
    char* input_buffer,
    char* output_buffer,
    uint64_t first_offset,
    int entry_num,
    ...
    for (int i = 1; i < entry_num; ++i)
        is_consistent &= (node_version == entries[i].h_version.node_version);
    return is_consistent;
}
